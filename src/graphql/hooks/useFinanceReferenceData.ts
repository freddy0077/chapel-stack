...
export function useFinanceReferenceData(branchId?: string) {
  const { data: fundsData, loading: fundsLoading, error: fundsError } = useQuery(GET_FUNDS, { variables: { branchId }, skip: !branchId });
  const { data: typesData, loading: typesLoading, error: typesError } = useQuery(GET_CONTRIBUTION_TYPES, { variables: { branchId }, skip: !branchId });
  const { data: methodsData, loading: methodsLoading, error: methodsError } = useQuery(GET_PAYMENT_METHODS, { variables: { branchId }, skip: !branchId });
