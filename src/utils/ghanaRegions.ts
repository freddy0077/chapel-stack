...
export const useGhanaRegions = () => {
  return {
    ...
    getRegionByCode: (code: string) => GHANA_REGIONS.find(region => region.code === code),
    getRegionByName: (name: string) => GHANA_REGIONS.find(region => region.name === name),
    getRegionOptions: () => GHANA_REGIONS.map(region => ({
      value: region.name,
      label: region.name,
      code: region.code,
