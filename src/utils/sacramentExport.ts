 ...
 */
export function convertToCSV(records: AnySacramentRecord[], includeFields?: string[]): string {
  ...
  const existingFields = fields.filter(field => 
    records.some(record => record.hasOwnProperty(field) && record[field as keyof AnySacramentRecord] != null)
  );

  // Create CSV header
  const header = existingFields.map(field => {
...
export async function exportToCSV(
  ...
  try {
    ...
    const filename = generateExportFilename('csv', 
      options.filters?.sacramentType === 'all' ? undefined : options.filters?.sacramentType,
      options.dateRange
    );
...
function generateHTMLTable(records: AnySacramentRecord[]): string {
  ...
  const fields = ['memberName', 'sacramentType', 'dateOfSacrament', 'locationOfSacrament', 'officiantName'];
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="export-info">
          ...
          ${options.filters?.sacramentType && options.filters.sacramentType !== 'all' 
...
export function filterRecordsForExport(
  ...
  if (options.filters?.sacramentType && options.filters.sacramentType !== 'all') {
    filteredRecords = filteredRecords.filter(record => 
      record.sacramentType === options.filters!.sacramentType
    );
  }
