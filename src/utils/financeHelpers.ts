 ...
 */
export const validateTransactionForm = (form: any): { isValid: boolean; message: string } => {
 ...
 */
export const calculateFinancialHealth = (balance: number, monthlyExpenses: number): {
  status: string;
  color: string;
  icon: string;
} => {
 ...
 */
export const buildTransactionFilters = (filters: TransactionFilters, organisationId: string, branchId: string) => {
  return {
    organisationId,
    branchId,
    type: filters.type === 'all' ? undefined : 
          filters.type === 'contribution' ? 'CONTRIBUTION' : 
          filters.type === 'expense' ? 'EXPENSE' : 
          filters.type === 'transfer' ? 'TRANSFER' : 
          filters.type === 'fund_allocation' ? 'FUND_ALLOCATION' : undefined,
 ...
 */
export const filterTransactionsBySearch = (transactions: any[], searchQuery: string) => {
 ...
 */
export const validateBatchEvents = (batchEvents: any[]): { isValid: boolean; message: string } => {
  const validEvents = batchEvents.filter(
    item => item.included && item.amount && !isNaN(Number(item.amount)) && Number(item.amount) > 0
  );
