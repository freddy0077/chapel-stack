...
export const SuperAdminSubscriptionManagement: React.FC<
  SuperAdminSubscriptionManagementProps
> = ({ organizationId }) => {
  const { user } = useAuth();
  ...
  const {
    ...
    refetch,
  } = useQuery(GET_ORGANIZATION_SUBSCRIPTION_STATUS, {
  ...
  const { createOrganizationSubscription, loading: creatingSubscription } =
    useCreateOrganizationSubscription();

  const subscription =
    subscriptionData?.organizationSubscriptionStatus?.subscription;
  const hasActiveSubscription =
    subscriptionData?.organizationSubscriptionStatus?.hasActiveSubscription;
  const isInGracePeriod =
    subscriptionData?.organizationSubscriptionStatus?.isInGracePeriod;
  const daysUntilExpiry =
    subscriptionData?.organizationSubscriptionStatus?.daysUntilExpiry;
      ...
      return {
        ...
        label:
          trialDaysRemaining > 0
            ...
            : `Trial expired ${Math.abs(trialDaysRemaining)} days ago`,
      };
    }
    ...
    return {
      ...
      label:
        subscriptionDaysRemaining > 0
          ...
          : `Expired ${Math.abs(subscriptionDaysRemaining)} days ago`,
    };
  };
  ...
  const getSubscriptionStatus = () => {
    if (!subscription)
      return { status: "NONE", color: "red", icon: XCircleIcon };
      ...
      <Card className="p-6">
              ...
              <div>
                ...
                {remainingDaysInfo && (
                  <div className="mt-1 space-y-1">
                    <p
                      className={`text-sm font-medium ${remainingDaysInfo.days > 0 ? "text-green-600" : "text-red-600"}`}
                    >
              ...
              <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                <div className="flex items-center">
                  ...
                  <p className="text-yellow-800">
                    Your subscription is in grace period. Please renew to avoid
        ...
        <div className="mt-6 flex gap-3">
          ...
          {statusInfo.status === "ACTIVE" && (
            <Button variant="outline" onClick={() => setShowRenewalModal(true)}>
              Upgrade Plan
            </Button>
          )}
        </div>
