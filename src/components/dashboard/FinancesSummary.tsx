...
export function FinancesSummary({
  ...
  const topBranchesData = {
    labels: topBranches?.map((b) => b.name) ?? [
      "Accra Central",
      "Kumasi",
      "Tema",
      "Cape Coast",
    ...
    datasets: [
      {
        label: "Giving (₵)",
        data: topBranches?.map((b) => b.amount) ?? [
          12000, 10500, 9000, 8700, 8000,
        ],
        backgroundColor: [
          "#2563eb",
          "#eab308",
  ...
  const expensesVsIncomeData = {
    labels: expensesVsIncome?.labels ?? [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
    ...
    datasets: [
      {
        label: "Income",
        data: expensesVsIncome?.income ?? [
          8000, 9000, 10000, 9500, 11000, 12000,
        ],
        borderColor: "#2563eb",
      ...
      {
        label: "Expenses",
        data: expensesVsIncome?.expenses ?? [
          6000, 7000, 8000, 7500, 9000, 9500,
        ],
        borderColor: "#eab308",
  ...
  return (
    <section className="bg-white rounded-2xl shadow-lg p-8 mb-8 border border-blue-100">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="flex flex-col">
          <span className="text-gray-500 text-sm">Top 5 Giving Branches</span>
          <span className="font-semibold text-blue-700">(Bar Chart)</span>
        </div>
        <div className="flex flex-col">
