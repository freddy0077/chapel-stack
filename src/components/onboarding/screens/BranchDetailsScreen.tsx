...
const BranchDetailsScreen: React.FC<BranchDetailsScreenProps> = ({
  ...
  const [expandedSection, setExpandedSection] = useState<
    ("primary" | "admin" | "settings" | "done")[]
  >(Array.from({ length: branches.length }, () => "primary"));

  // Field handlers
  const handleBranchChange = (
    idx: number,
    field: keyof BranchDetailsInput,
    value: string,
  ) => {
    setBranches((prev) => {
  ...
  const handleSettingsChange = (
    idx: number,
    field: keyof BranchSettingsInput,
    value: string,
  ) => {
    setBranches((prev) =>
      prev.map((b, i) =>
    ...
    setBranches((prev) =>
      prev.map((b, i) =>
        i === idx
          ? {
              ...b,
              modules: b.modules.includes(module)
  ...
  const handleSaveSettings = async (idx: number) => {
    ...
    try {
      ...
      for (const { key, value } of updates) {
        await createBranchSetting({
          variables: { input: { branchId, key, value } },
        });
      }
  ...
  useEffect(() => {
    if (primarySaved.every(Boolean))
      markScreenCompleted("BranchPrimaryDetails");
    ...
    <div className="space-y-8 flex flex-col items-center w-full">
      <div className="mb-8 text-center w-full max-w-screen-md">
        ...
        <p className="text-gray-600">
          Fill in the details for each branch, create an admin, and configure
          ...
          >
            ...
            >
              ...
              <span className="font-bold text-lg">
                Branch {idx + 1} of {branches.length}
              </span>
              <span className="ml-auto text-sm font-semibold">
                {branch.branchName ? branch.branchName : "Not named yet"}
              </span>
              <span className="ml-4 text-xs font-medium opacity-70">
                {openIdx === idx ? "▼" : "▶"}
              </span>
            </button>
            <div
                  ...
                  {expandedSection[idx] === "primary" ? (
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleBranchChange(idx, "address", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <select
                            ...
                            value={branch.status || ""}
                            onChange={(e) =>
                              handleBranchChange(idx, "status", e.target.value)
                            }
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={branch.city || ""}
                            onChange={(e) =>
                              handleBranchChange(idx, "city", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={branch.phone || ""}
                            onChange={(e) =>
                              handleBranchChange(idx, "phone", e.target.value)
                            }
                            placeholder="e.g. +233 123 456 789"
                          />
                          {branch.phone && !isValidPhone(branch.phone) && (
                            <div className="text-red-500 text-xs mt-1">
                      ...
                      <Button
                        type="button"
                        color="indigo"
                        onClick={() => handleSavePrimary(idx)}
                        className="w-full md:w-auto mt-4"
                        disabled={!primaryValid[idx] || primaryLoading[idx]}
                      >
                        {primaryLoading[idx]
                          ? "Saving..."
                      ...
                      {primarySaved[idx] && (
                        <div className="text-green-600 mt-2">
                          Primary details saved successfully!
                        </div>
                      )}
                      {primaryError[idx] && (
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input
                          ...
                          onChange={(e) =>
                            handleAdminChange(idx, "firstName", e.target.value)
                          }
                          required
                        />
                        <input
                          ...
                          onChange={(e) =>
                            handleAdminChange(idx, "lastName", e.target.value)
                          }
                          required
                        />
                        <input
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleAdminChange(idx, "password", e.target.value)
                          }
                          required
                        />
                      </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {moduleOptions.map((module) => (
                              <label
                                key={module}
                                className="inline-flex items-center bg-indigo-50 px-2 py-1 rounded-md hover:bg-indigo-100"
                              >
                                <input
                                  type="checkbox"
                                  checked={branch.modules.includes(module)}
                                  onChange={() =>
                                    handleModuleToggle(idx, module)
                                  }
