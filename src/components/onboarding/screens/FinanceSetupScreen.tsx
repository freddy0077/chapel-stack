...
const FINANCE_IMPORT_TYPES = [
  {
    key: "fund",
    label: "Funds/Accounts",
    sampleHeaders: ["Name", "Type", "Opening Balance", "Currency"],
    sampleData: [["Main Bank", "Bank", "10000", "USD"]],
  },
  {
    key: "contributionType",
    label: "Contribution Types",
    sampleHeaders: ["Name", "Description"],
    sampleData: [["Tithe", "Tithes and offerings"]],
  },
  {
    key: "contribution",
    label: "Contributions",
  ...
  },
  {
    ...
    sampleHeaders: ["Name", "Description"],
    sampleData: [["Utilities", "Electricity, water, etc."]],
  },
  {
    key: "expense",
    label: "Expenses",
  ...
  },
  {
    key: "vendor",
    label: "Vendors",
    sampleHeaders: ["Name", "Contact"],
    sampleData: [["ECG", "0200000000"]],
  },
  {
    key: "paymentMethod",
  ...
  },
  {
    key: "budget",
    label: "Budgets",
    sampleHeaders: ["Name", "Year", "Total"],
    sampleData: [["2024 Budget", "2024", "50000"]],
  },
  {
    key: "pledge",
    label: "Pledges",
    sampleHeaders: ["Date", "Amount", "Member", "Fund", "Status"],
    sampleData: [
      ["2024-01-10", "1000", "Jane Smith", "Missions Fund", "Pending"],
    ],
  },
];

const FinanceSetupScreen: React.FC<FinanceSetupScreenProps> = ({
  onNext,
  onBack,
  onSkip,
  isLoading,
  ...
  const [uploads, setUploads] = useState<
    Record<string, { fileName: string; previewRows: unknown[][] }>
  >({});
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);

  const handleFileChange =
    (key: string, headers: string[], sampleData: unknown[]) =>
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setUploads((u) => ({
        ...u,
        [key]: { fileName: file.name, previewRows: [headers, ...sampleData] },
      }));
    };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    saveOnboardingStepData("FinanceSetup", {
      uploadedFiles: Object.keys(uploads)
        .map((key) => uploads[key]?.fileName)
        .filter(Boolean),
    });
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] relative">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-50">
        <div className="bg-yellow-200 bg-opacity-90 border-4 border-yellow-400 rounded-2xl px-12 py-10 shadow-2xl flex flex-col items-center max-w-2xl">
          <span className="text-5xl md:text-7xl font-extrabold text-yellow-700 tracking-widest mb-4">
            UNDER DEVELOPMENT
          </span>
          <span className="text-lg text-yellow-800 font-semibold mb-4 text-center">
            This page will allow you to import and set up all your church’s
            financial data, including accounts, contribution types, donations,
            expenses, vendors, payment methods, budgets, and pledges. You’ll be
            able to upload spreadsheets or connect to accounting software to
            migrate your records easily.
          </span>
          <span className="text-base text-yellow-900 mb-6 text-center">
            We’re working hard to bring you a seamless finance onboarding
            experience. In the meantime, you can skip this step and complete
            your setup later.
          </span>
          {onSkip && (
            <button
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-gray-100 p-8 opacity-40 blur-sm pointer-events-none select-none">
        ...
        <p className="mb-6 text-gray-600">
          Import your financial data by uploading spreadsheets for each finance
          type below. Download the sample template for each, fill in your data,
          and upload. You can import as many as you want now, or skip and do it
          later.
        </p>
        <form onSubmit={handleSubmit} className="space-y-8">
          {FINANCE_IMPORT_TYPES.map((type) => (
            <div
              key={type.key}
              className="mb-4 border-b pb-6 last:border-b-0 last:pb-0"
            >
              <div className="flex items-center justify-between">
                <label className="block text-base font-semibold text-indigo-700">
                  {type.label}
                </label>
                <a
                  href="#"
              ...
              {uploads[type.key]?.fileName && (
                <span className="block text-sm text-gray-500 mt-1">
                  Selected: {uploads[type.key].fileName}
                </span>
              )}
                ...
                <div className="mt-2 border rounded-lg overflow-x-auto">
                  <table className="min-w-full text-xs">
                    <thead className="bg-indigo-50">
                      <tr>
                        {uploads[type.key].previewRows[0].map(
                          (header: string, i: number) => (
                            <th
                              key={i}
                              className="px-2 py-1 font-semibold text-indigo-700"
                            >
                              {header}
                            </th>
                          ),
