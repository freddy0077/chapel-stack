...
import { GET_EVENT_REGISTRATIONS, GET_EVENT_REGISTRATION_STATS } from '../../graphql/mutations/eventMutations';
...
const EventAttendeesView: React.FC<EventAttendeesViewProps> = ({
  ...
  const { data: registrationsData, loading, error, refetch } = useQuery(GET_EVENT_REGISTRATIONS, {
  ...
  const stats = statsData?.eventRegistrationStats ? JSON.parse(statsData.eventRegistrationStats) : null;
  ...
  const exportToCSV = () => {
    const headers = ['Name', 'Email', 'Phone', 'Status', 'Registration Date', 'Check-in Time', 'Guests', 'Special Requests'];
    const csvData = filteredRegistrations.map(reg => [
      ...
      reg.checkInTime ? format(new Date(reg.checkInTime), 'yyyy-MM-dd HH:mm') : '',
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 text-white">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-100">{format(new Date(eventDate), 'EEEE, MMMM d, yyyy')}</p>
            </div>
            <button
              onClick={onClose}
        ...
        {stats && (
          <div className="px-6 py-4 bg-gray-50 border-b">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              ...
              <div className="bg-white rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-indigo-600">{stats.checkedIn}</div>
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              <div className="flex flex-wrap gap-4">
                <div>
                  ...
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value as EventRegistrationStatus | 'ALL')}
                    className="border border-gray-300 rounded-md px-3 py-2 text-sm"
                  >
                    <option value="ALL">All Statuses</option>
                    <option value={EventRegistrationStatus.REGISTERED}>Registered</option>
                    <option value={EventRegistrationStatus.CONFIRMED}>Confirmed</option>
                    <option value={EventRegistrationStatus.ATTENDED}>Attended</option>
                    <option value={EventRegistrationStatus.NO_SHOW}>No Show</option>
                    <option value={EventRegistrationStatus.CANCELLED}>Cancelled</option>
                    <option value={EventRegistrationStatus.WAITLISTED}>Waitlisted</option>
                  </select>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {error ? (
            <div className="p-6 text-center">
              ...
              <p className="text-red-600">Error loading attendees: {error.message}</p>
            </div>
                ...
                <div key={registration.id} className="p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2">
                          ...
                          {!registration.member && (
                            <Badge color="gray" size="xs">Guest</Badge>
                          )}
                        </div>
                        <div className="flex items-center space-x-4 mt-1">
                          <p className="text-sm text-gray-500">
                            {registration.member?.email || registration.guestEmail || 'No email'}
                          </p>
                        ...
                        {registration.checkInTime && (
                          <p className="text-xs text-green-600 mt-1">
                            Checked in: {format(new Date(registration.checkInTime), 'MMM d, h:mm a')}
                          </p>
                        )}
                    ...
                    <div className="flex items-center space-x-3">
                      <Badge className={getStatusColor(registration.status)} size="sm">
                        {registration.status.replace('_', ' ')}
                      </Badge>
                      <div className="text-right">
                        <p className="text-xs text-gray-500">
                          Registered: {format(new Date(registration.registrationDate), 'MMM d')}
                        </p>
                      </div>
                    </div>
                  </div>
