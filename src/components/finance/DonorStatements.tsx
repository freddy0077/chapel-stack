...
interface DonorStatementsProps {
  donors: DonorData[];
  loading?: boolean;
  onGenerateStatement?: (donorId: string, format: 'pdf' | 'email') => void;
  onBulkGenerate?: (donorIds: string[], format: 'pdf' | 'email') => void;
}
...
export default function DonorStatements({ 
  ...
  const handlePreviewStatement = (donor: DonorData) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* Bulk Actions */}
        {selectedDonors.length > 0 && onBulkGenerate && (
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-600">
              {selectedDonors.length} selected
            </span>
            <button
              onClick={() => onBulkGenerate(selectedDonors, 'pdf')}
            ...
            <button
              onClick={() => onBulkGenerate(selectedDonors, 'email')}
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        >
          {selectedDonors.length === filteredDonors.length ? 'Deselect All' : 'Select All'}
        </button>
      </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {onGenerateStatement && (
                    <>
                      <button
                        onClick={() => onGenerateStatement(donor.id, 'pdf')}
                        className="px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 flex items-center gap-2"
                      >
                        <ArrowDownTrayIcon className="h-4 w-4" />
                        PDF
                      ...
                      <button
                        onClick={() => onGenerateStatement(donor.id, 'email')}
      ...
      {filteredDonors.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            {searchTerm ? 'Try adjusting your search terms.' : 'No donors with giving history available.'}
          </p>
        </div>
      )}
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              ...
              <div className="flex items-center gap-2">
                {onGenerateStatement && (
                  <>
                    <button
                      onClick={() => onGenerateStatement(selectedDonor.id, 'pdf')}
                      className="px-3 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 flex items-center gap-2"
                    >
                      <ArrowDownTrayIcon className="h-4 w-4" />
                      Download PDF
                    ...
                    <button
                      onClick={() => onGenerateStatement(selectedDonor.id, 'email')}
                      className="px-3 py-2 text-sm font-medium text-green-600 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-green-500"
                    >
                      Email Statement
                    </button>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {selectedDonor.address && (
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{selectedDonor.address}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-green-50 rounded-lg p-6 mb-6">
                <h3 className="font-semibold text-green-900 mb-4">Giving Summary</h3>
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 text-center text-sm text-gray-600">
                ...
                <p className="mt-2">Thank you for your generous support of our ministry!</p>
              </div>
            </div>
          </div>
        </div>
