...
import { MEMBER_GIVING_ANALYSIS, MemberGivingAnalysisInput, MemberGivingAnalysisResult } from '../../graphql/queries/analytics';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-2xl border border-indigo-100 shadow-2xl w-full max-w-6xl mx-4 p-0 relative animate-fadeIn max-h-[90vh] overflow-hidden">
        ...
        <div className="px-6 py-6 max-h-[calc(90vh-120px)] overflow-y-auto">{children}</div>
      </div>
    </div>
  );
}

export default function MemberGivingHistory({ isOpen, onClose }: MemberGivingHistoryProps) {
  ...
  const { data: memberResults, loading: membersLoading } = useQuery(GET_MEMBERS_LIST, {
    variables: {
      organisationId,
      branchId,
  ...
  const { data: givingAnalysisData, loading: analysisLoading, error: analysisError } = useQuery<{ memberGivingAnalysis: MemberGivingAnalysisResult }>(
  ...
  const getTrendDisplay = (direction: string, changePercent: number) => {
    ...
    return {
      icon: isPositive ? ArrowTrendingUpIcon : isNegative ? ArrowTrendingDownIcon : ChartBarIcon,
  ...
  return (
    <Modal open={isOpen} title="Member Giving History & Analytics" onClose={onClose}>
            ...
            <div className="mt-3 p-3 bg-indigo-50 rounded-lg flex items-center justify-between">
              <div>
                ...
                {selectedMember.email && (
                  <div className="text-sm text-indigo-600">{selectedMember.email}</div>
                )}
              </div>
              <button
            ...
            {analysisError && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <p className="text-red-700 text-sm">
                  {analysisError.message || 'Unable to load member giving analytics. Please try again.'}
                </p>
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-blue-800">Contributions</p>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-purple-800">Giving Rank</p>
                  ...
                  <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-orange-800">YoY Change</p>
                        <p className="text-2xl font-bold text-orange-900">
                          {analysisData.yearOverYearChange >= 0 ? '+' : ''}{analysisData.yearOverYearChange.toFixed(1)}%
                        </p>
                      </div>
                ...
                {analysisData.givingTrend && (
                  <div className="bg-white border border-gray-200 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 mb-3">Giving Trend Analysis</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="text-center">
                        <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-full ${getTrendDisplay(analysisData.givingTrend.direction, analysisData.givingTrend.changePercent).bgColor}`}>
                          {React.createElement(getTrendDisplay(analysisData.givingTrend.direction, analysisData.givingTrend.changePercent).icon, {
                            className: `h-4 w-4 ${getTrendDisplay(analysisData.givingTrend.direction, analysisData.givingTrend.changePercent).color}`
                          })}
                          <span className={`text-sm font-medium ${getTrendDisplay(analysisData.givingTrend.direction, analysisData.givingTrend.changePercent).color}`}>
                      ...
                      <div className="text-center">
                        <p className="text-lg font-bold text-gray-900">
                          {getTrendDisplay(analysisData.givingTrend.direction, analysisData.givingTrend.changePercent).text}
                        </p>
                ...
                {analysisData.fundBreakdown.length > 0 && (
                  <div className="bg-white border border-gray-200 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 mb-3">Giving by Fund</h4>
                    <div className="space-y-3">
                      {analysisData.fundBreakdown.map((fund, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div>
                            <p className="font-medium text-gray-900">{fund.fundName}</p>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-200">
                            ...
                            <th className="text-left py-2 font-medium text-gray-900">Fund</th>
                            <th className="text-left py-2 font-medium text-gray-900">Description</th>
                          </tr>
                        </thead>
                        <tbody>
                          {analysisData.recentContributions.map((contribution, index) => (
        ...
        {!selectedMember && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              Choose a member from the search above to view their detailed giving history and analytics.
            </p>
          </div>
        )}
      </div>
