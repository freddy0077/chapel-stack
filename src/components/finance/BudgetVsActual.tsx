...
import { BUDGET_VS_ACTUAL_QUERY, BudgetVsActual as BudgetVsActualType, BudgetVsActualInput } from '@/graphql/queries/budgetVsActual';
import { 
  ChartBarIcon, 
  ArrowTrendingUpIcon, 
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <div className="text-red-600 text-center">
          ...
          <h3 className="text-lg font-medium mb-2">Error Loading Budget Analysis</h3>
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <div className="text-gray-500 text-center">
          ...
          <p className="text-sm">Budget vs Actual analysis will appear here when data is available.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="mt-4 sm:mt-0">
            <select
              ...
              onChange={(e) => setSelectedPeriodType(e.target.value as 'MONTHLY' | 'QUARTERLY' | 'YEARLY')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-green-50 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-green-600">Total Actual</p>
          ...
          {/* Variance */}
          <div className={`rounded-lg p-4 ${budgetData.summary.totalVariance >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
            <div className="flex items-center">
              <div className="flex-shrink-0">
                {getVarianceIcon(budgetData.summary.totalVariance)}
              </div>
              <div className="ml-4">
                <p className={`text-sm font-medium ${budgetData.summary.totalVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  Variance
                </p>
                <p className={`text-2xl font-bold ${budgetData.summary.totalVariance >= 0 ? 'text-green-900' : 'text-red-900'}`}>
                  {formatCurrency(budgetData.summary.totalVariance)}
                </p>
                <p className={`text-xs ${budgetData.summary.totalVariance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center">
                        ...
                        <span className={`ml-1 ${item.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center">
                        {getVarianceIcon(item.variance)}
                        <span className={`ml-1 ${item.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {formatCurrency(Math.abs(item.variance))} ({formatPercentage(Math.abs(item.variancePercent))})
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
