...
interface SearchableMemberOrTextInputProps {
  value?: string;
  ...
  label?: string;
  error?: string;
  allowTextInput?: boolean; // Allow manual text input as fallback
}

export default function SearchableMemberOrTextInput({
  value,
  onChange,
  ...
  error,
  allowTextInput = true,
}: SearchableMemberOrTextInputProps) {
  ...
  useEffect(() => {
    if (value && !selectedMember && !isTextMode) {
      setTextValue(value);
      setSearchTerm(value);
    }
  }, [value, selectedMember, isTextMode]);
