...
export const ModernDeathRegisterForm: React.FC<
  ModernDeathRegisterFormProps
> = ({
  deathRegister,
  isOpen,
  onClose,
  ...
  const handleInputChange = (
    field: keyof DeathRegisterFormData,
    value: any,
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submitData = {
        ...
        dateOfDeath:
          formatDateToISO(formData.dateOfDeath) || formData.dateOfDeath, // Fallback to original if formatting fails
        funeralDate: formatDateToISO(formData.funeralDate),
        notificationDate:
          formData.familyNotified && formData.notificationDate
      ...
      Object.keys(submitData).forEach((key) => {
        ...
        if (
          submitData[key] === "" &&
          ["branchId", "funeralEventId"].includes(key)
        ) {
          delete submitData[key];
        }
      });
          ...
          {currentStep === 0 && (
            <MemberSelectionStep
              ...
              onSelectMember={(member) =>
                handleInputChange("selectedMember", member)
              }
    ...
    {isEditMode && selectedMember ? (
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <div>
            ...
            <div className="text-sm text-gray-600 space-y-1">
              ...
              {selectedMember.phoneNumber && (
                <p>Phone: {selectedMember.phoneNumber}</p>
              )}
        ...
        <div className="mt-3 text-sm text-blue-700 bg-blue-100 rounded-lg p-3">
          ...
          <p>
            Member selection cannot be changed when editing an existing death
            record. To change the member, please create a new record.
          </p>
        </div>
      </div>
    ) : (
