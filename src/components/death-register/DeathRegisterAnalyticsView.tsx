...
const DeathRegisterAnalyticsView: React.FC<DeathRegisterAnalyticsViewProps> = ({
  ...
  const monthlyData = useMemo(() => {
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const burialCount = monthRecords.filter(
        (r) => r.burialCremation === "BURIAL",
      ).length;
      const cremationCount = monthRecords.filter(
        (r) => r.burialCremation === "CREMATION",
      ).length;
      const avgAge =
          ...
          ? monthRecords.reduce((sum, r) => {
              if (r.member?.dateOfBirth) {
                const age =
                  ...
                  new Date(r.member.dateOfBirth).getFullYear();
                return sum + age;
              }
              return sum;
  ...
  const ageDistributionData = useMemo(() => {
    ...
    records.forEach((record) => {
      if (record.member?.dateOfBirth) {
        const age =
          new Date(record.dateOfDeath).getFullYear() -
          new Date(record.member.dateOfBirth).getFullYear();
  ...
  const burialCremationData = useMemo(() => {
    ...
    return [
      ...
      {
        name: "Cremation",
        value: stats.cremationCount,
        color: COLORS.cremation,
      },
    ];
  }, [stats]);
  ...
  const keyMetrics = useMemo(() => {
    ...
    const thisYearRecords = records.filter(
      (r) => new Date(r.dateOfDeath).getFullYear() === currentYear,
    );
    const lastYearRecords = records.filter(
      (r) => new Date(r.dateOfDeath).getFullYear() === lastYear,
    );

    const yearOverYearChange =
      lastYearRecords.length > 0
        ? Math.round(
            ((thisYearRecords.length - lastYearRecords.length) /
              ...
              <PieChart>
                <Pie
                  ...
                  label={({ name, value, percentage }) =>
                    `${name}: ${value} (${Math.round((value / stats.total) * 100)}%)`
                  }
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="value"
          ...
          <div className="mt-4 flex justify-center space-x-6">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">
                Burial ({stats.burialCount})
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-4 h-4 rounded-full bg-yellow-500"></div>
              <span className="text-sm text-gray-600">
                Cremation ({stats.cremationCount})
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className="font-medium text-gray-900">
                  Family Notification Rate
                </p>
                <p className="text-sm text-gray-600">
                  {Math.round((stats.familyNotifiedCount / stats.total) * 100)}%
                  of families have been notified
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {stats.funeralServicesHeld} funeral services have been
                  conducted (
                  {Math.round((stats.funeralServicesHeld / stats.total) * 100)}%
                  of total deaths)
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {Math.round(
                    (stats.burialCount /
                      (stats.burialCount + stats.cremationCount)) *
                      100,
                  )}
                  % choose burial,
                  {Math.round(
                    (stats.cremationCount /
                      (stats.burialCount + stats.cremationCount)) *
                      100,
                  )}
                  % choose cremation
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  The average age at time of death is{" "}
