...
import { DeathRegisterFormData, BurialType } from "../../types/deathRegister";
...
export const DeathDetailsStep: React.FC<StepProps> = ({
  formData,
  onChange,
  errors,
}) => (
...
export const FuneralArrangementsStep: React.FC<StepProps> = ({
  formData,
  onChange,
  errors,
}) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
      ...
      />
      {errors.funeralDate && (
        <p className="mt-1 text-sm text-red-600">{errors.funeralDate}</p>
      )}
    </div>
    ...
    <div>
      ...
      {errors.funeralLocation && (
        <p className="mt-1 text-sm text-red-600">{errors.funeralLocation}</p>
      )}
    </div>
    ...
    <div>
      ...
      <select
        ...
        onChange={(e) =>
          onChange("burialCremation", e.target.value as BurialType)
        }
        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
      >
...
export const FamilyContactStep: React.FC<StepProps> = ({
  formData,
  onChange,
  errors,
}) => (
  <div className="space-y-6">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        />
        {errors.nextOfKin && (
          <p className="mt-1 text-sm text-red-600">{errors.nextOfKin}</p>
        )}
      </div>
    ...
    <div>
      ...
      <div className="flex items-center space-x-3">
        <input
          ...
          onChange={(e) =>
            onFileSelect?.(e.target.files?.[0] || null, "deathCertificateUrl")
          }
        />
        <button
          type="button"
          onClick={() =>
            document.getElementById("death-certificate-upload")?.click()
          }
        ...
        >
          ...
          {uploading.deathCertificate
            ? `Uploading... (${uploadProgress.deathCertificate}%)`
    ...
    <div>
      ...
      <div className="flex items-center space-x-3">
        <input
          ...
          id="obituary-upload"
          onChange={(e) =>
            onFileSelect?.(e.target.files?.[0] || null, "obituaryUrl")
          }
        />
        <button
          type="button"
        ...
        >
          ...
          {uploading.obituary
            ? `Uploading... (${uploadProgress.obituary}%)`
            : "Upload Obituary"}
        </button>
    ...
    <div>
      ...
      <div className="flex items-center space-x-3 mb-3">
        <input
          ...
          id="photos-upload"
          onChange={(e) =>
            onFileSelect?.(e.target.files?.[0] || null, "photoUrls")
          }
        />
        <button
          type="button"
        ...
        >
          ...
          {uploading.photos
            ? `Uploading... (${uploadProgress.photos}%)`
            : "Upload Photos"}
        </button>
        {formData.photoUrls && formData.photoUrls.length > 0 && (
          <span className="ml-3 text-sm text-green-600 flex items-center">
            <CheckCircleIcon className="h-5 w-5 mr-1" />{" "}
            {formData.photoUrls.length} photo(s) uploaded
          </span>
        )}
      </div>
      {uploading.photos && (
    ...
    <div>
      ...
      <div className="flex items-center space-x-3 mb-3">
        <input
          ...
          onChange={(e) =>
            onFileSelect?.(e.target.files?.[0] || null, "additionalDocuments")
          }
        />
        <button
          type="button"
        ...
        >
          ...
          {uploading.additionalDocuments
            ? `Uploading... (${uploadProgress.additionalDocuments}%)`
            : "Upload Documents"}
        </button>
        {formData.additionalDocuments &&
          formData.additionalDocuments.length > 0 && (
            <span className="ml-3 text-sm text-green-600 flex items-center">
              <CheckCircleIcon className="h-5 w-5 mr-1" />{" "}
              {formData.additionalDocuments.length} document(s) uploaded
            </span>
          )}
      </div>
      {uploading.additionalDocuments && (
            ...
            {formData.additionalDocuments.map((doc, index) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => {
                    const newDocs = formData.additionalDocuments.filter(
                      (_, i) => i !== index,
                    );
