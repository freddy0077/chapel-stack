...
export const OrganizationSubscriptionDashboard: React.FC<
  OrganizationSubscriptionDashboardProps
> = ({ organizationId, showLifecycleStats = false }) => {
  const { user } = useAuth();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          ...
          {showLifecycleStats && (
            ...
            >
              <Cog6ToothIcon
                className={`h-4 w-4 mr-2 ${lifecycleCheckLoading ? "animate-spin" : ""}`}
              />
              Run Lifecycle Check
            </Button>
          )}
      ...
      {!hasActiveSubscription && (
        <Alert variant="destructive">
          ...
          <div>
            ...
            <p>
              Your organization does not have an active subscription. Please
      ...
      {isExpiringSoon && !isInGracePeriod && (
        <Alert variant="warning">
          ...
          <div>
            ...
            <p>
              Your subscription expires in {daysUntilExpiry} days. Please renew
              to avoid service interruption.
            </p>
          </div>
        </Alert>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-gray-900">
                {lifecycleStats.totalSubscriptions}
              </p>
              <p className="text-sm text-gray-500">Total</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">
                {lifecycleStats.activeSubscriptions}
              </p>
              <p className="text-sm text-gray-500">Active</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">
                {lifecycleStats.trialSubscriptions}
              </p>
              <p className="text-sm text-gray-500">Trial</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-yellow-600">
                {lifecycleStats.pastDueSubscriptions}
              </p>
              <p className="text-sm text-gray-500">Past Due</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-red-600">
                {lifecycleStats.cancelledSubscriptions}
              </p>
              <p className="text-sm text-gray-500">Cancelled</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-orange-600">
                {lifecycleStats.expiringIn7Days}
              </p>
              <p className="text-sm text-gray-500">Expiring (7d)</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">
                {lifecycleStats.expiringIn30Days}
              </p>
