...
interface SacramentLoadingManager {
  ...
  setSacramentLoading: (sacramentType: SacramentType, loading: boolean, operation?: string) => void;
  setModalLoading: (modalType: keyof SacramentLoadingManager['modalLoading'], loading: boolean, operation?: string, recordId?: string) => void;
  ...
  withLoading: <T>(
    operation: () => Promise<T>,
    loadingType: 'global' | SacramentType | keyof SacramentLoadingManager['modalLoading'],
...
export function useSacramentLoading(): SacramentLoadingManager {
  ...
  const [sacramentLoading, setSacramentLoadingState] = useState<Record<SacramentType, LoadingState>>(initialSacramentLoading);
  ...
  const setGlobalLoading = useCallback((loading: boolean, operation?: string) => {
  ...
  const setSacramentLoading = useCallback((sacramentType: SacramentType, loading: boolean, operation?: string) => {
  ...
  const isAnyLoading = useCallback(() => {
    ...
    if (Object.values(sacramentLoading).some(state => state.isLoading)) return true;
    
    if (Object.values(modalLoading).some(state => state.isLoading)) return true;
    
    return false;
  }, [globalLoading, sacramentLoading, modalLoading]);

  const isOperationLoading = useCallback((operation: string) => {
    return activeOperations.current.has(operation) ||
  ...
  const withLoading = useCallback(async <T>(
    ...
    try {
      ...
      } else if (Object.values(SACRAMENT_TYPES).includes(loadingType as SacramentType)) {
        setSacramentLoading(loadingType as SacramentType, true, operationName);
      } else {
        setModalLoading(loadingType as keyof SacramentLoadingManager['modalLoading'], true, operationName, recordId);
      }
      ...
      if (loadingType === 'global') {
        ...
        setModalLoading(loadingType as keyof SacramentLoadingManager['modalLoading'], false);
      }
    }
