...
export const useCreateOrganizationSubscription = () => {
  ...
  const createOrganizationSubscription = useCallback(async (
...
export const useOrganizationSubscriptionStatus = (organizationId?: string) => {
  ...
  const subscriptionStatus = useMemo(() => 
    data?.organizationSubscriptionStatus as OrganizationSubscriptionStatus | undefined, 
    [data?.organizationSubscriptionStatus]
  );

  const hasActiveSubscription = useMemo(() => subscriptionStatus?.hasActiveSubscription ?? false, [subscriptionStatus?.hasActiveSubscription]);
  const isInGracePeriod = useMemo(() => subscriptionStatus?.isInGracePeriod ?? false, [subscriptionStatus?.isInGracePeriod]);
  const daysUntilExpiry = useMemo(() => subscriptionStatus?.daysUntilExpiry, [subscriptionStatus?.daysUntilExpiry]);
  const subscription = useMemo(() => subscriptionStatus?.subscription, [subscriptionStatus?.subscription]);
  const isExpiringSoon = useMemo(() => (subscriptionStatus?.daysUntilExpiry ?? 999) <= 7, [subscriptionStatus?.daysUntilExpiry]);
  const isExpired = useMemo(() => (subscriptionStatus?.daysUntilExpiry ?? 999) < 0, [subscriptionStatus?.daysUntilExpiry]);
  ...
  return useMemo(() => ({
    ...
    isExpiringSoon,
    isExpired,
  }), [subscriptionStatus, loading, error, stableRefetch, hasActiveSubscription, isInGracePeriod, daysUntilExpiry, subscription, isExpiringSoon, isExpired]);
};
...
export const useSubscriptionLifecycleStats = () => {
  ...
  const stats = useMemo(() => 
    data?.subscriptionLifecycleStats as SubscriptionLifecycleStats | undefined,
    [data?.subscriptionLifecycleStats]
  );

  const totalSubscriptions = useMemo(() => stats?.totalSubscriptions ?? 0, [stats?.totalSubscriptions]);
  const activeSubscriptions = useMemo(() => stats?.activeSubscriptions ?? 0, [stats?.activeSubscriptions]);
  const trialSubscriptions = useMemo(() => stats?.trialSubscriptions ?? 0, [stats?.trialSubscriptions]);
  const pastDueSubscriptions = useMemo(() => stats?.pastDueSubscriptions ?? 0, [stats?.pastDueSubscriptions]);
  const cancelledSubscriptions = useMemo(() => stats?.cancelledSubscriptions ?? 0, [stats?.cancelledSubscriptions]);
  const expiringIn7Days = useMemo(() => stats?.expiringIn7Days ?? 0, [stats?.expiringIn7Days]);
  const expiringIn30Days = useMemo(() => stats?.expiringIn30Days ?? 0, [stats?.expiringIn30Days]);
  const healthySubscriptions = useMemo(() => (stats?.activeSubscriptions ?? 0) + (stats?.trialSubscriptions ?? 0), [stats?.activeSubscriptions, stats?.trialSubscriptions]);
  const atRiskSubscriptions = useMemo(() => (stats?.pastDueSubscriptions ?? 0) + (stats?.expiringIn7Days ?? 0), [stats?.pastDueSubscriptions, stats?.expiringIn7Days]);
...
export const useSubscriptionLifecycleCheck = () => {
  ...
  const triggerLifecycleCheck = useCallback(async (): Promise<SubscriptionLifecycleResult> => {
  ...
  const result = useMemo(() => 
    data?.triggerSubscriptionLifecycleCheck as SubscriptionLifecycleResult | undefined,
...
export const useOrganizationSubscriptionManagement = (organizationId?: string) => {
  ...
  return useMemo(() => ({
    ...
    // Creation
    createOrganizationSubscription: createSubscription.createOrganizationSubscription,
    ...
    // Utilities
    refreshAll,
    loading: subscriptionStatus.loading || createSubscription.loading || lifecycleStats.loading,
    error: subscriptionStatus.error || createSubscription.error || lifecycleStats.error,
  }), [
