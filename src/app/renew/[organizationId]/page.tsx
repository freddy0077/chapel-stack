...
export default function OrganizationRenewalPage() {
  ...
  const [step, setStep] = useState<
    "verify" | "select-plan" | "payment" | "success" | "error"
  >("verify");
  ...
  const organization =
    orgData?.organizationSubscriptionStatus?.subscription?.organisation;
  ...
  useEffect(() => {
    if (orgData && !orgLoading) {
      const hasActiveSubscription =
        orgData.organizationSubscriptionStatus?.hasActiveSubscription;
      ...
      paystackService.openPaystackPopup(
        ...
        async (response: any) => {
          try {
            ...
            setErrorMessage(
              "Payment was successful but subscription creation failed. Please contact support.",
            );
            setStep("error");
          }
        },
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="max-w-md w-full mx-4 p-6 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            The organization ID provided is invalid or the organization does not
            exist.
          </p>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {step === "select-plan" && (
          <div>
            <div className="text-center mb-8">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Your organization's subscription has expired. Please select a
                plan to renew access.
              </p>
            </div>
            ...
            <div className="space-y-4 max-w-4xl mx-auto">
              {plans.map((plan) => (
                <Card
                  key={plan.id}
                  className="p-6 hover:shadow-lg transition-shadow cursor-pointer border-2 hover:border-indigo-500"
        ...
        {step === "payment" && (
          <div>
            <div className="text-center mb-8">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Provide your contact details to complete the subscription
                renewal.
              </p>
            </div>
        ...
        {step === "success" && (
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Your organization's subscription has been successfully renewed.
              You can now log in to access the system.
            </p>
            <Button
              className="mt-4"
              onClick={() =>
                router.push("/auth/signin?message=renewal-success")
              }
            >
              Go to Login
            </Button>
        ...
        {step === "error" && (
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-500 max-w-md mx-auto">
              {errorMessage ||
                "An error occurred while processing your renewal. Please try again or contact support."}
            </p>
