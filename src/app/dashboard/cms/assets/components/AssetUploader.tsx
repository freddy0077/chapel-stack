...
export default function AssetUploader({
  ...
  const [uploadStatus, setUploadStatus] = useState<AssetUploadStatus | null>(null);
  ...
  const handleFileSelect = (file: File) => {
    ...
    const titleFromFilename = file.name.replace(/\.[^/.]+$/, '').replace(/-/g, ' ');
  ...
  const getFileTypeIcon = (fileType: string) => {
    if (fileType.startsWith('image/')) return (
      <div className="h-6 w-6 text-indigo-500">📷</div>
    );
    ...
    return (
      <DocumentIcon className="h-6 w-6 text-pink-500" />
    );
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const metadata: Partial<MediaAsset> = {
        ...
        permissions: {
          public: isPublic,
          branches: branch === 'all' ? branches.map(b => b.id).filter(id => id !== 'all') : [branch],
          roles: []
        }
      };
                ...
                {selectedFile ? (
                  <div className="text-center">
                    ...
                    <div className="flex items-center justify-center">
                      ...
                      <span className="font-medium text-green-700">File selected</span>
                    </div>
                    <Text className="text-gray-500 mt-1">{selectedFile.name}</Text>
                  ...
                  <>
                    ...
                    <Text className="text-xs text-gray-500 mt-1">
                      Supports: {allowedTypes.join(', ')} up to {formatFileSize(maxFileSize)}
                    </Text>
                  </>
                )}
              </div>
              ...
              {uploadStatus && (
                <div className="mt-4">
                  <div className="flex justify-between text-sm mb-1">
                    <span>{uploadStatus.status.charAt(0).toUpperCase() + uploadStatus.status.slice(1)}</span>
            ...
            <div>
              <div className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="public-access" className="ml-2 block text-sm text-gray-700">
