...
export default function BackgroundChecksPage() {
  ...
  const [backgroundChecks, setBackgroundChecks] = useState(MOCK_BACKGROUND_CHECKS);

  // Filter background checks based on search, branch, and status
  const filteredChecks = backgroundChecks.filter(check => {
    // Branch filter
    if (selectedBranch !== 'all' && check.branch !== BRANCHES.find(b => b.id === selectedBranch)?.name) {
      return false;
    }
    
    // Status filter
    if (selectedStatus !== 'all' && check.status.toLowerCase() !== selectedStatus.toLowerCase()) {
      return false;
    }
  ...
  const pendingChecks = backgroundChecks.filter(check => check.status === 'Pending').length;
  const expiredChecks = backgroundChecks.filter(check => check.status === 'Expired').length;
  const compliantChecks = backgroundChecks.filter(check => check.status === 'Completed').length;
  ...
  const getStatusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      case 'rejected':
        return <XCircleIcon className="h-5 w-5 text-gray-500" aria-hidden="true" />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Background Checks</h1>
      ...
      <div className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd>
                    <div className="text-lg font-medium text-gray-900">{complianceRate}%</div>
                  </dd>
                </dl>
              </div>
            </div>
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="p-5">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Pending Checks</dt>
                  <dd>
                    <div className="text-lg font-medium text-gray-900">{pendingChecks}</div>
                  </dd>
                </dl>
              </div>
            </div>
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="p-5">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Expired Checks</dt>
                  <dd>
                    <div className="text-lg font-medium text-gray-900">{expiredChecks}</div>
                  </dd>
                </dl>
              </div>
            </div>
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="p-5">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <BuildingOfficeIcon className="h-6 w-6 text-indigo-400" aria-hidden="true" />
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Total Volunteers</dt>
                  <dd>
                    <div className="text-lg font-medium text-gray-900">{totalChecks}</div>
                  </dd>
                </dl>
              </div>
            </div>
              ...
              {filteredChecks.map((check) => (
                <tr key={check.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{check.volunteerName}</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <span className={`ml-1.5 inline-flex px-2 text-xs font-semibold rounded-full ${getStatusBadgeColor(check.status)}`}>
                        {check.status}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    {check.completedDate && <div>Completed: {formatDate(check.completedDate)}</div>}
                    {check.expirationDate && <div>Expires: {formatDate(check.expirationDate)}</div>}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div>{check.branch}</div>
                    <div className="text-xs text-gray-400">{check.teams.join(', ')}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex space-x-2">
                      <Link href={`/dashboard/volunteers/background-checks/${check.id}`} className="text-indigo-600 hover:text-indigo-900">
                        View
                      </Link>
      ...
      <div className="mt-8 bg-white shadow overflow-hidden rounded-lg">
        <div className="px-4 py-5 sm:p-6">
          <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Policy & Resources</h3>
          <div className="space-y-4">
            <Link href="/dashboard/volunteers/resources/background-policy" className="block hover:bg-gray-50 p-3 rounded-lg transition-colors duration-150">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span className="ml-3 text-gray-900 font-medium">Background Check Policy Document</span>
                </div>
                <ChevronRightIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
              </div>
            </Link>
            
            <Link href="/dashboard/volunteers/resources/faq" className="block hover:bg-gray-50 p-3 rounded-lg transition-colors duration-150">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <svg className="h-6 w-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span className="ml-3 text-gray-900 font-medium">Frequently Asked Questions</span>
                </div>
                <ChevronRightIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
              </div>
            </Link>
            
            <Link href="/dashboard/volunteers/resources/training" className="block hover:bg-gray-50 p-3 rounded-lg transition-colors duration-150">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <svg className="h-6 w-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span className="ml-3 text-gray-900 font-medium">Safety Training Resources</span>
                </div>
                <ChevronRightIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
              </div>
            </Link>
