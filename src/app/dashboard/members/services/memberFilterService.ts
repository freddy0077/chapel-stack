...
export class MemberFilterService {
  ...
  static extractBackendFilters(filters: MemberFilters): any {
    ...
    if (filters.organisationId)
      backendFilters.organisationId = filters.organisationId;
    if (filters.hasMemberId !== undefined)
      backendFilters.hasMemberId = filters.hasMemberId;
    ...
    if (filters.maritalStatus && filters.maritalStatus.length > 0) {
      const validMaritalStatus = filters.maritalStatus.filter(
        (ms) => ms != null && ms !== "",
      );
      if (validMaritalStatus.length > 0)
        backendFilters.maritalStatus = validMaritalStatus;
    }

    if (filters.membershipStatus && filters.membershipStatus.length > 0) {
      const validMembershipStatus = filters.membershipStatus.filter(
        (ms) => ms != null && ms !== "",
      );
      if (validMembershipStatus.length > 0)
        backendFilters.membershipStatus = validMembershipStatus;
    }

    if (filters.memberStatus && filters.memberStatus.length > 0) {
      const validMemberStatus = filters.memberStatus.filter(
        (ms) => ms != null && ms !== "",
      );
      if (validMemberStatus.length > 0)
        backendFilters.memberStatus = validMemberStatus;
    }

    // Handle boolean filters
    if (filters.hasProfileImage !== undefined)
      backendFilters.hasProfileImage = filters.hasProfileImage;
    if (filters.hasEmail !== undefined)
      backendFilters.hasEmail = filters.hasEmail;
  ...
  static generateFilterStats(members: Member[]) {
    ...
    members.forEach((member) => {
      ...
      stats.maritalStatus.set(
        maritalStatus,
        (stats.maritalStatus.get(maritalStatus) || 0) + 1,
      );

      // Membership status stats
      const membershipStatus = member.membershipStatus || "Not Specified";
      stats.membershipStatus.set(
        membershipStatus,
        (stats.membershipStatus.get(membershipStatus) || 0) + 1,
      );

      // Member status stats
      const memberStatus = member.memberStatus || "ACTIVE";
      stats.memberStatus.set(
        memberStatus,
        (stats.memberStatus.get(memberStatus) || 0) + 1,
      );
  ...
  static getActiveFilterCount(filters: MemberFilters): number {
    ...
    if (
      ...
      count++;
    if (filters.dateRange?.joinedAfter || filters.dateRange?.joinedBefore)
      count++;
