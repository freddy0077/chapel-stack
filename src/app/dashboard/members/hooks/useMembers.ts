...
import { GET_MEMBERS_LIST, GET_MEMBER_STATISTICS, SEARCH_MEMBERS_ENHANCED } from '@/graphql/queries/memberQueries';
import { 
  Member, 
  MemberFilters, 
  MemberSearchParams, 
...
export const useMembers = (options: UseMembersOptions = {}): UseMembersReturn => {
  ...
  }, [data?.membersCount, searchData?.searchMembers, currentSearch, filteredMembers.length]);
...
export const useMemberStatistics = (branchId?: string, organisationId?: string) => {
  const { organisationId: currentOrganisationId, branchId: currentBranchId } = useOrganisationBranch();
  ...
  const {
    ...
    error,
    refetch
  } = useQuery(GET_MEMBER_STATISTICS, {
    variables: {
