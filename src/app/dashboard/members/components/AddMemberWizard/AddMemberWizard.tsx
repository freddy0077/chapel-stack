...
const WIZARD_STEPS = [
  ...
  { id: 5, title: 'Communication Preferences', component: CommunicationPrefsStep },
...
const AddMemberWizard: React.FC<AddMemberWizardProps> = ({
  ...
  const getCurrentStepComponent = () => {
    ...
    if (!StepComponent) {
      ...
      return <div className="p-4 text-red-500">Error: Step component not found</div>;
    }
    ...
    try {
      ...
      return <div className="p-4 text-red-500">Error rendering step: {error.message}</div>;
    }
  };

  if (!isOpen) return null;

  console.log('AddMemberWizard rendering, isOpen:', isOpen, 'currentStep:', currentStep);
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        >
          ...
          <div className="mb-4 p-2 bg-yellow-100 text-xs text-yellow-800 rounded">
            Debug: Step {currentStep} of {WIZARD_STEPS.length} | FormData keys: {Object.keys(formData).length}
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Step {currentStep} of {WIZARD_STEPS.length}: {WIZARD_STEPS.find(s => s.id === currentStep)?.title}
              </p>
            </div>
            <button
              onClick={handleClose}
