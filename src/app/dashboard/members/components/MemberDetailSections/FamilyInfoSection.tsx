...
const FamilyInfoSection: React.FC<FamilyInfoSectionProps> = ({ member }) => {
  const hasParentInfo = member.fatherName || member.motherName || member.fatherOccupation || member.motherOccupation;
  const hasEmergencyContact = member.emergencyContactName || member.emergencyContactPhone || member.emergencyContactRelation;
  const hasFamilyStatus = member.headOfHousehold !== undefined || member.familyId;
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="flex items-center mb-4">
        ...
        <h3 className="ml-3 text-lg font-semibold text-gray-900">Family Information</h3>
      </div>
      ...
      <div className="space-y-6">
        ...
        {hasFamilyStatus && (
          <div>
            <h4 className="text-md font-medium text-gray-900 mb-3">Family Status</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {member.headOfHousehold !== undefined && (
                <div>
                  <label className="text-sm font-medium text-gray-500">Head of Household</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {member.fatherOccupation && (
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <p className="text-sm text-gray-900">{member.fatherOccupation}</p>
                  </div>
                </div>
              )}
              ...
              {member.motherOccupation && (
                <div>
                  ...
                  <div className="flex items-center">
                    <BriefcaseIcon className="w-4 h-4 text-gray-400 mr-2" />
                    <p className="text-sm text-gray-900">{member.motherOccupation}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {member.emergencyContactRelation && (
                <div>
                  ...
                  <p className="text-sm text-gray-900">{member.emergencyContactRelation}</p>
                </div>
              )}
