...
export default function CreateMarriageModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateMarriageModalProps) {
  const { state } = useAuth();
  ...
  const {
    createMatrimonyRecord,
    loading: mutationLoading,
    error: mutationError,
  } = useCreateMatrimonyRecord();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data?.createSacramentalRecord) {
        ...
        setErrorMessage(
          "Failed to create marriage record. Please check all required fields.",
        );
      }
    } catch (error) {
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-rose-100 text-sm">
                          Sacred union of two hearts in love
                        </p>
                      </div>
                    </div>
                    <button
