...
export default function SacramentAnalytics({
  className = "",
}: SacramentAnalyticsProps) {
  ...
  const stats: SacramentStats = useMemo(() => {
    ...
    return {
      ...
      totalMarriages,
      totalSacraments:
        totalBaptisms + totalCommunions + totalConfirmations + totalMarriages,
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `sacrament-analytics-${selectedPeriod.toLowerCase()}-${new Date().toISOString().split("T")[0]}.csv`,
      );
  ...
  const generateCSVReport = (stats: SacramentStats, period: string) => {
    ...
    const rows = [
      ...
      [
        "Baptisms",
        stats.totalBaptisms.toString(),
        stats.baptismTrend.toString(),
      ],
      [
        "First Communions",
        stats.totalCommunions.toString(),
        stats.communionTrend.toString(),
      ],
      [
        "Confirmations",
        stats.totalConfirmations.toString(),
        stats.confirmationTrend.toString(),
      ],
      [
        "Marriages",
        stats.totalMarriages.toString(),
        stats.marriageTrend.toString(),
      ],
    ];
      ...
      >
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 mb-4">
            Unable to fetch sacrament analytics data.
          </p>
          <button
            onClick={() => refetch()}
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          {sacramentTypes.map((type) => {
            ...
            const trend = stats[
              type.trendKey as keyof SacramentStats
            ] as number;
            ...
            return (
              <div
                key={type.key}
                className={`${type.bgColor} border ${type.borderColor} rounded-lg p-4`}
              >
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className={`text-sm font-medium ${type.iconColor}`}>
                      {type.name}
                    </p>
                    <p
                      className={`text-2xl font-bold ${type.iconColor.replace("600", "900")}`}
                    >
                      {count}
                    </p>
                  </div>
                  <div
                    className={`p-3 ${type.bgColor.replace("50", "200")} rounded-full`}
                  >
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 w-full flex flex-col">
                      <div
                        ...
                        style={{
                          height: `${(month.baptisms / Math.max(...stats.monthlyData.map((m) => m.baptisms + m.communions + m.confirmations + m.marriages))) * 200}px`,
                        }}
                      ></div>
                      <div
                        className="bg-amber-500"
                        style={{
                          height: `${(month.communions / Math.max(...stats.monthlyData.map((m) => m.baptisms + m.communions + m.confirmations + m.marriages))) * 200}px`,
                        }}
                      ></div>
                      <div
                        className="bg-purple-500"
                        style={{
                          height: `${(month.confirmations / Math.max(...stats.monthlyData.map((m) => m.baptisms + m.communions + m.confirmations + m.marriages))) * 200}px`,
                        }}
                      ></div>
                      <div
                        className="bg-rose-500"
                        style={{
                          height: `${(month.marriages / Math.max(...stats.monthlyData.map((m) => m.baptisms + m.communions + m.confirmations + m.marriages))) * 200}px`,
                        }}
                      ></div>
                    </div>
                  </div>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-blue-900 mb-2">
              Top Performing Sacrament
            </h4>
            <p className="text-blue-800">
              {
                sacramentTypes.reduce((prev, current) =>
                  (stats[prev.key as keyof SacramentStats] as number) >
                  (stats[current.key as keyof SacramentStats] as number)
                    ? prev
                    : current,
                ).name
              }{" "}
              ...
              {Math.max(
                ...sacramentTypes.map(
                  (type) => stats[type.key as keyof SacramentStats] as number,
                ),
              )}{" "}
              records
            </p>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <p className="text-green-800">
              {
                sacramentTypes.filter(
                  (type) =>
                    (stats[type.trendKey as keyof SacramentStats] as number) >
                    0,
                ).length
              }{" "}
              out of 4 sacraments showing positive growth
            </p>
          </div>
        </div>
      </div>
