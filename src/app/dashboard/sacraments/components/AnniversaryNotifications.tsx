...
export default function AnniversaryNotifications({ className = "" }: AnniversaryNotificationsProps) {
  ...
  const filteredAnniversaries = anniversaries.filter(anniversary => {
    ...
    const daysDiff = Math.ceil((anniversaryDate.getTime() - today.getTime()) / (1000 * 3600 * 24));
  ...
  const handleSendNotification = async (anniversaryId: string, method: 'email' | 'sms') => {
    ...
    try {
      ...
      toast.success(`${method === 'email' ? 'Email' : 'SMS'} notification sent successfully`);
    } catch (error) {
    ...
    return (
      <div className={`bg-white rounded-lg shadow-sm border border-gray-200 p-6 ${className}`}>
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">Unable to fetch anniversary notifications.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Anniversary Notifications</h2>
              <p className="text-sm text-gray-600">Manage sacrament anniversary reminders</p>
            </div>
          </div>
      ...
      <div className="p-6">
        {filteredAnniversaries.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Anniversaries</h3>
          ...
          <div className="space-y-4">
            {filteredAnniversaries.map((anniversary) => {
              ...
              const isOverdue = new Date(anniversary.anniversaryDate) < new Date() && !anniversary.notificationSent;
              ...
              return (
                <div
                  ...
                  className={`border rounded-lg p-4 ${
                    ...
                    anniversary.notificationSent ? 'border-green-200 bg-green-50' :
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {anniversary.notificationScheduled && !anniversary.notificationSent && (
                        ...
                        <p className="text-sm text-gray-600 mb-2">
                          {getOrdinalSuffix(anniversary.yearsAgo)} {formatSacramentType(anniversary.sacramentType)} Anniversary
                        </p>
                        
                        <div className="flex items-center space-x-4 text-sm text-gray-500">
                          <div className="flex items-center space-x-1">
                            <CalendarDaysIcon className="h-4 w-4" />
                            <span>{new Date(anniversary.anniversaryDate).toLocaleDateString()}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                        ...
                        <div className="mt-2">
                          <span className={`text-sm font-medium ${
                            ...
                            {getDaysUntilAnniversary(anniversary.anniversaryDate)}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      {!anniversary.notificationSent && (
                        <>
                          {anniversary.memberEmail && (
                            <button
                              onClick={() => handleSendNotification(anniversary.id, 'email')}
                            ...
                            >
                              ...
                              {isSending === anniversary.id ? 'Sending...' : 'Email'}
                            </button>
                          )}
                          
                          {anniversary.memberPhone && (
                            <button
                              onClick={() => handleSendNotification(anniversary.id, 'sms')}
                              disabled={isSending === anniversary.id}
                            ...
                            >
                              ...
                              {isSending === anniversary.id ? 'Sending...' : 'SMS'}
                            </button>
                          )}
                          
                          {!anniversary.notificationScheduled ? (
                            <button
                              onClick={() => handleScheduleNotification(anniversary.id)}
                            ...
                            >
                              <ClockIcon className="h-3 w-3 mr-1" />
                              {isScheduling === anniversary.id ? 'Scheduling...' : 'Schedule'}
                            </button>
                          ) : (
                            <button
                              onClick={() => handleCancelScheduled(anniversary.id)}
