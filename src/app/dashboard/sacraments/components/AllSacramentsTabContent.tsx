...
export default function AllSacramentsTabContent({
  ...
  const currentSacrament = SACRAMENT_TYPES.find(
    (s) => s.key === selectedSacramentType,
  );
  ...
  useEffect(() => {
    if (refetch && currentSacrament) {
      ...
      const refetchRef =
        refetchRefMap[
          currentSacrament.refetchRef as keyof typeof refetchRefMap
        ];
      if (refetchRef) {
  ...
  const handleCreateRecord = () => {
    ...
    const handler =
      handlerMap[currentSacrament.onClick as keyof typeof handlerMap];
    if (handler) {
      handler();
    }
  };
  ...
  const getRecords = () => {
    ...
    if (data.getFilteredBaptismRecords)
      return data.getFilteredBaptismRecords.records || [];
    if (data.getFilteredCommunionRecords)
      return data.getFilteredCommunionRecords.records || [];
    if (data.getFilteredConfirmationRecords)
      return data.getFilteredConfirmationRecords.records || [];
    if (data.getFilteredMarriageRecords)
      return data.getFilteredMarriageRecords.records || [];
        ...
        {loading ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Loading {currentSacrament.name.toLowerCase()} records...
            </p>
          </div>
        ) : error ? (
          <div className="p-8 text-center">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Records for {currentSacrament.name.toLowerCase()} are not yet
