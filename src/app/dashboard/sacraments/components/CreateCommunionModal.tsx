...
interface CreateCommunionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export default function CreateCommunionModal({ isOpen, onClose, onSuccess }: CreateCommunionModalProps) {
  const { state } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createFirstCommunionRecord({
        variables: {
          input: {
            memberId: formData.memberId,
            sacramentType: "EUCHARIST_FIRST_COMMUNION",
            dateOfSacrament: new Date(formData.communionDate),
            locationOfSacrament: formData.location,
            officiantName: formData.officiant,
            sponsorName: formData.sponsors || null,
      ...
      setErrorMessage("Failed to create communion record. Please try again.");
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            >
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-amber-100 text-sm">First reception of the Holy Eucharist</p>
                      </div>
                    </div>
                    <button
                      onClick={onClose}
                ...
                <div className="bg-gray-50 px-6 py-4 flex items-center justify-end space-x-3">
                  ...
                  >
                    {isSubmitting ? "Creating..." : "Create Communion Record"}
                  </button>
                </div>
              </Dialog.Panel>
            </Transition.Child>
