...
const getSecondaryInfo = (record: SacramentRecord): string[] => {
  ...
  switch (record.sacramentType) {
    case "BAPTISM":
      if (record.godparent1Name)
        info.push(`Godparent: ${record.godparent1Name}`);
      if (record.godparent2Name)
        info.push(`Godparent: ${record.godparent2Name}`);
      break;
    case "EUCHARIST_FIRST_COMMUNION":
    case "CONFIRMATION":
...
const getMemberDisplayInfo = (
  member: Member,
): { age?: number; displayGender?: string } => {
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white shadow-xl transition-all">
                ...
                {/* Content */}
                <div
                  className={`p-6 ${record.sacramentType === "MATRIMONY" ? "bg-gradient-to-b from-rose-50/50 to-white" : ""}`}
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Basic Information */}
                    <div className="space-y-4">
                      <h3
                        className={`text-lg font-semibold ${record.sacramentType === "MATRIMONY" ? "text-rose-900" : "text-gray-900"} border-b ${record.sacramentType === "MATRIMONY" ? "border-rose-200" : "border-gray-200"} pb-2 flex items-center space-x-2`}
                      >
                        {record.sacramentType === "MATRIMONY" && (
                      ...
                      <div className="flex items-center space-x-3">
                        <CalendarIcon
                          className={`h-5 w-5 ${record.sacramentType === "MATRIMONY" ? "text-rose-400" : "text-gray-400"}`}
                        />
                        <div>
                          <p
                            className={`text-sm font-medium ${record.sacramentType === "MATRIMONY" ? "text-rose-900" : "text-gray-900"}`}
                          >
                            {record.sacramentType === "MATRIMONY"
                              ? "Wedding Date"
                              : "Date"}
                          ...
                          <p
                            className={`text-sm ${record.sacramentType === "MATRIMONY" ? "text-rose-700" : "text-gray-600"}`}
                          >
                            {format(
                              new Date(record.dateOfSacrament),
                              "MMMM d, yyyy",
                    ...
                    <div className="space-y-4">
                      ...
                      {(record.sacramentType === "EUCHARIST_FIRST_COMMUNION" ||
                        record.sacramentType === "CONFIRMATION") && (
                        <div className="space-y-3">
                          {record.sponsorName && (
                            <div>
                              <p className="text-sm font-medium text-gray-900">
                      ...
                      {record.member && (
                        <div className="space-y-3">
                          <h4
                            className={`text-sm font-semibold ${record.sacramentType === "MATRIMONY" ? "text-rose-900" : "text-gray-900"} flex items-center space-x-2`}
                          >
                            {record.sacramentType === "MATRIMONY" && (
                          ...
                          {record.member.profileImageUrl && (
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                ...
                                {(() => {
                                  const { age, displayGender } =
                                    getMemberDisplayInfo(record.member!);
                                  const details = [
                                    age ? `Age: ${age}` : null,
                                    displayGender,
                                  ].filter(Boolean);
                          ...
                          {/* Age and Gender */}
                          {(() => {
                            const { age, displayGender } = getMemberDisplayInfo(
                              record.member!,
                            );
                            return age || displayGender ? (
                          ...
                          {record.member.membershipDate && (
                            <div>
                              ...
                              <p className="text-sm text-gray-600">
                                {format(
                                  new Date(record.member.membershipDate),
                                  "MMMM d, yyyy",
                                )}
                              </p>
                            </div>
