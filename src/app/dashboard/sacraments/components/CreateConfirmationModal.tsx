...
export default function CreateConfirmationModal({ isOpen, onClose, onSuccess }: CreateConfirmationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createConfirmationRecord({
        variables: {
          input: {
            memberId: formData.memberId,
            sacramentType: "CONFIRMATION",
            dateOfSacrament: new Date(formData.confirmationDate),
      ...
      if (result.data?.createSacramentalRecord) {
        setSuccessMessage("Confirmation record created successfully!");
        setTimeout(() => {
          onSuccess?.();
          onClose();
        }, 1500);
