...
import {
  exportSacramentRecords,
  filterRecordsForExport,
} from "@/utils/sacramentExport";
...
export default function SacramentsPage() {
  ...
  const handleCertificateGenerated = useCallback(
    (certificateUrl: string, certificateNumber: string) => {
  ...
  const handleViewMemberMarriageHistory = useCallback(
    (memberId: string) => {
      ...
      try {
        await loadingManager.withLoading(
          async () => {
            ...
            toast.success(
              `${recordIds.length} sacrament record(s) deleted successfully`,
            );
            refetchManager.refetchAll();
          },
          "delete",
  ...
  const handleBulkExport = useCallback(
    async (recordIds: string[], format: "csv" | "pdf" | "excel") => {
      try {
        await loadingManager.withLoading(
          async () => {
            ...
            toast.success(
              `${recordIds.length} record(s) exported as ${format.toUpperCase()}`,
            );
          },
          "export",
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        <DashboardHeader
          ...
          action={
            <SacramentActionMenu
              onBaptismClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.BAPTISM)
              }
              onCommunionClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.COMMUNION)
              }
              onConfirmationClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.CONFIRMATION)
              }
              onMarriageClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.MARRIAGE)
              }
              onReconciliationClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.RECONCILIATION)
              }
              onAnointingClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.ANOINTING)
              }
              onDiaconateClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.DIACONATE)
              }
              onPriesthoodClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.PRIESTHOOD)
              }
              onRciaClick={() =>
                modalManager.openSacramentModal(SACRAMENT_TYPES.RCIA)
              }
            />
          }
        />
                ...
                <div className="space-y-6">
                  ...
                  <SacramentTabContentEnhanced
                    ...
                    confirmationRefetchRef={
                      refetchManager.confirmationRefetchRef
                    }
                    marriageRefetchRef={refetchManager.marriageRefetchRef}
                    reconciliationRefetchRef={
                      refetchManager.reconciliationRefetchRef
                    }
                    ...
                    onViewMemberMarriageHistory={
                      handleViewMemberMarriageHistory
                    }
                  />
                </div>
              )}
        ...
        <SacramentModalManager
          ...
          setIsBaptismModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.BAPTISM)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.BAPTISM)
          }
          setIsCommunionModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.COMMUNION)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.COMMUNION)
          }
          setIsConfirmationModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.CONFIRMATION)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.CONFIRMATION)
          }
          setIsMarriageModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.MARRIAGE)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.MARRIAGE)
          }
          setIsReconciliationModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.RECONCILIATION)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.RECONCILIATION)
          }
          setIsAnointingModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.ANOINTING)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.ANOINTING)
          }
          setIsDiaconateModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.DIACONATE)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.DIACONATE)
          }
          setIsPriesthoodModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.PRIESTHOOD)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.PRIESTHOOD)
          }
          setIsRciaModalOpen={(open) =>
            open
              ? modalManager.openSacramentModal(SACRAMENT_TYPES.RCIA)
              : modalManager.closeSacramentModal(SACRAMENT_TYPES.RCIA)
          }
          baptismRefetchRef={refetchManager.baptismRefetchRef}
          communionRefetchRef={refetchManager.communionRefetchRef}
          ...
          setIsMarriageAnalyticsOpen={(open) =>
            open
              ? modalManager.openMarriageAnalytics()
              : modalManager.closeModal(MODAL_TYPES.MARRIAGE_ANALYTICS)
          }
          isMemberMarriageHistoryOpen={modalManager.isMemberMarriageHistoryOpen}
          setIsMemberMarriageHistoryOpen={(open) =>
            open
              ? null
              : modalManager.closeModal(MODAL_TYPES.MEMBER_MARRIAGE_HISTORY)
          }
              ...
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 rounded-lg">
                  <h4 className="font-medium text-blue-900 mb-2">
                    {formatSacramentType(
                      modalManager.selectedRecord.sacramentType,
                    )}{" "}
                    Certificate
                  </h4>
                  <p className="text-sm text-blue-700 mb-3">
                    Member:{" "}
                    {modalManager.selectedRecord.memberName ||
                      `ID: ${modalManager.selectedRecord.memberId}`}
                  </p>
                  <p className="text-sm text-blue-700">
                    Date:{" "}
                    {new Date(
                      modalManager.selectedRecord.dateOfSacrament,
                    ).toLocaleDateString()}
                  </p>
                </div>
