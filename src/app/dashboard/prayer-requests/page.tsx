...
export default function PrayerRequestsPage() {
  ...
  const {
    prayerRequests: allRequests,
    loading,
    error,
    refetch,
  } = usePrayerRequests(organisationId, branchId);
  ...
  const filteredRequests = allRequests.filter((prayer: PrayerRequest) => {
    ...
    const matchesStatus =
      statusFilter === null || prayer.status === statusFilter;
  ...
  const statusCounts = {
    ...
    inProgress: allRequests.filter(
      (p: PrayerRequest) => p.status === "IN_PROGRESS",
    ).length,
    answered: allRequests.filter((p: PrayerRequest) => p.status === "ANSWERED")
      .length,
  };
          ...
          <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-gray-500 mb-6">
              {searchTerm || statusFilter
                ...
                : "Start by adding a new prayer request"}
            </p>
            <button
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex flex-col">
                        <h3 className="font-semibold text-indigo-700 text-lg">
                          {prayer.member?.firstName || "-"}{" "}
                          {prayer.member?.lastName || ""}
                        </h3>
                      ...
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full border text-sm font-medium ${statusConfig[prayer.status].color}`}
                      >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <span
                          className={`inline-flex items-center gap-1 px-3 py-1 rounded-full border text-sm font-semibold ${statusConfig[prayer.status].color}`}
                        >
                          {statusConfig[prayer.status].icon}
                          {statusConfig[prayer.status].text}
                        </span>
