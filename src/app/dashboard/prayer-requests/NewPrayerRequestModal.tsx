...
export default function NewPrayerRequestModal({
  open,
  onClose,
  onSuccess,
}: NewPrayerRequestModalProps) {
  const { state } = useAuth();
  ...
  const [createPrayerRequest, { loading: submitting, error }] = useMutation(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canManagePrayerRequests) {
      if (!user?.member?.id) {
        toast.error(
          "Unable to create prayer request: Your member profile is not set up",
        );
        return;
      }
                    ...
                    <div className="relative">
                      ...
                      {touched && !requestText.trim() && (
                        <p className="mt-1 text-sm text-red-600">
                          Prayer request text is required
                        </p>
                      )}
