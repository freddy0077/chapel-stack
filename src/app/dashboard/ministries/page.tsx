...
function AddMinistryModal({ onClose, onSuccess, branchId, afterCreate }: { onClose: () => void; onSuccess: () => void; branchId: string; afterCreate?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-300/30 via-white/40 to-indigo-100/40 backdrop-blur-[6px]" onClick={onClose} />
      <div className="relative bg-white/90 shadow-2xl rounded-3xl max-w-lg w-full p-8 border border-indigo-100 animate-fadeInUp">
        ...
        <div className="flex flex-col items-center gap-2 mb-6">
          ...
          <h2 className="text-2xl font-extrabold text-indigo-700">Add Ministry</h2>
...
export default function Ministries() {
  ...
  const [ministryRefetch, setMinistryRefetch] = useState<(() => void) | null>(null);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white pb-16 overflow-visible">
      ...
      <div className="relative isolate overflow-hidden bg-gradient-to-r from-indigo-600 to-indigo-400 px-6 py-12 shadow-lg rounded-b-3xl mb-8">
        <div className="mx-auto max-w-7xl flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-white drop-shadow-lg">Ministries</h1>
            <p className="mt-2 text-lg text-indigo-100 max-w-xl">Organize, manage, and grow all ministries and groups in your church. Add new ministries, assign leaders, and track membership at a glance.</p>
          </div>
          <button
        ...
        <div className="pointer-events-none absolute inset-0 -z-10 opacity-40" aria-hidden="true">
          <svg className="absolute -top-6 left-1/2 -translate-x-1/2" width="1024" height="200" fill="none"><ellipse cx="512" cy="100" rx="512" ry="100" fill="#fff" fillOpacity="0.05"/></svg>
        </div>
      </div>
      ...
      <MinistryLoader>
        {(ministries, loading, error, refetch) => {
          ...
          if (loading) {
            return (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-4 md:px-8">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="animate-pulse bg-white/80 rounded-2xl h-44 shadow-inner" />
                ))}
              </div>
            );
          }
          ...
          if (!ministries || ministries.length === 0) {
            return (
              <div className="flex flex-col items-center justify-center py-24">
                <img src="/empty-state-ministries.svg" alt="No ministries" className="h-40 mb-6 opacity-80" />
                <p className="text-xl text-gray-400 mb-2">No ministries found.</p>
                <button
                  onClick={() => setAddModalOpen(true)}
          ...
          const filteredMinistries = ministries.filter((m: Ministry) => {
            ...
            const typeMatch = typeFilter === "all" || (m.type || "GENERAL").toUpperCase() === typeFilter;
                    ...
                    >
                      <div className="p-6 pb-4">
                        <div className="flex items-center gap-3 mb-2">
                          <span className="text-xl font-bold text-indigo-700 truncate flex-1">{ministry.name}</span>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${ministry.status === 'Inactive' ? 'bg-gray-200 text-gray-500' : 'bg-indigo-100 text-indigo-700'}`}>{ministry.status || 'Active'}</span>
                        </div>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="inline-block px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-500 text-xs font-medium">{ministry.type || 'General'}</span>
                        </div>
                        <div className="flex items-center gap-2 mt-2">
                          <UsersIcon className="h-4 w-4 text-gray-400" />
                          <span className="text-sm text-gray-600">{ministry.members?.length || 0} members</span>
                          {ministry.members && ministry.members.length > 0 && (
                            <div className="flex ml-2 -space-x-2">
                              {ministry.members.slice(0, 3).map((member: MinistryMember, idx: number) => (
                                <img
                                  key={member.memberId || idx}
                                  src={member.avatarUrl || '/avatar-default.png'}
                                  alt={member.name || 'Member'}
                                  className="h-7 w-7 rounded-full border-2 border-white shadow -ml-1"
                                />
                              ))}
                              {ministry.members.length > 3 && (
                                <span className="ml-2 text-xs text-gray-400 font-medium">+{ministry.members.length - 3} more</span>
                              )}
                            </div>
                          )}
                        </div>
