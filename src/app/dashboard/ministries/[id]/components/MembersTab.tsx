...
export default function MembersTab({ ministryId }: MembersTabProps) {
  ...
  const [selectedMember, setSelectedMember] = useState<MinistryMember | null>(
    null,
  );

  // Check if user has permission to manage members
  const canManageMembers = hasPermission(
    `ministry:${ministryId}:manage_members`,
  );
                        ...
                        <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                          <div className="flex items-center">
                            <div className="w-full bg-gray-200 rounded-full h-2.5">
                              <div
                                className={`h-2.5 rounded-full ${
                                  selectedMember.attendance >= 90
                                    ? "bg-green-500"
                                    : selectedMember.attendance >= 75
                                      ? "bg-yellow-500"
                                      : "bg-red-500"
                                }`}
                                style={{
                                  width: `${selectedMember.attendance}%`,
                                }}
                              ></div>
                            </div>
                      ...
                      <>
                        <button
                          ...
                          onClick={() => {
                            ...
                            alert(
                              `Edit ${selectedMember.name} (not implemented)`,
                            );
                          }}
                        >
                          Edit Member
