  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-yellow-50 px-2 py-8">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold">
              Attendance Streak: {attendanceStreak} weeks
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          icon={<CalendarIcon className="w-7 h-7 text-blue-500" />}
          label="Upcoming Events"
          value={upcomingEvents.length}
        />
        <StatCard
          icon={<UserGroupIcon className="w-7 h-7 text-emerald-500" />}
          label="My Groups"
          value={myGroups.length}
        />
        <StatCard
          icon={<GiftIcon className="w-7 h-7 text-yellow-500" />}
          label="Giving"
          value={"GHS 0"}
        />
        <StatCard
          icon={<ArrowRightIcon className="w-7 h-7 text-gray-400" />}
          label="Recent Sacraments"
          value={"-"}
        />
      </div>
        ...
        <ul className="divide-y divide-blue-100">
          {upcomingEvents.map((event) => (
            ...
            >
              ...
              <button className="px-3 py-1 rounded bg-blue-100 text-blue-700 font-semibold text-xs hover:bg-blue-200 transition">
                View
              </button>
            </li>
          ))}
      ...
      {/* My Groups */}
      <SectionCard
        title="My Groups & Ministries"
        icon={<UserGroupIcon className="w-5 h-5 text-emerald-400" />}
      >
        <ul className="divide-y divide-emerald-100">
          {myGroups.map((group) => (
            ...
            >
              ...
              <button className="px-3 py-1 rounded bg-emerald-100 text-emerald-700 font-semibold text-xs hover:bg-emerald-200 transition">
                Details
              </button>
            </li>
          ))}
      ...
      <SectionCard
        title="Quick Links"
        icon={<ArrowRightIcon className="w-5 h-5 text-gray-400" />}
      >
        <div className="flex flex-wrap gap-4">
          {quickLinks.map((link) => (
            <a
              key={link.title}
              href={link.href}
              className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl shadow hover:bg-blue-50 border border-blue-100 transition"
            >
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-5 flex flex-col items-start gap-2 border border-blue-100 hover:shadow-xl transition group cursor-pointer relative overflow-hidden">
      ...
      <div className="text-2xl font-extrabold text-blue-800 drop-shadow-sm">
        {value}
      </div>
