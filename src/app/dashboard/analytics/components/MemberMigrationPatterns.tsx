...
const formatBranchMigrationData = (selectedBranch) => {
  ...
  return latestMigrationData.filter(
    (item) => item.from === selectedBranch || item.to === selectedBranch,
  );
};
...
export default function MemberMigrationPatterns() {
  ...
  const reasonsChartData = Object.entries(selectedBranchReasons).map(
    ([reason, count]) => ({
      reason,
      count,
    }),
            ...
            <Card className="mt-6">
              ...
              <BarChart
                ...
                valueFormatter={(value) =>
                  `${value > 0 ? "+" : ""}${value} members`
                }
                showLegend={false}
                showAnimation={true}
              />
              <Text className="text-xs text-gray-500 mt-2">
                Positive values indicate net gain; negative values indicate net
                loss
              </Text>
            </Card>
          </TabPanel>
              ...
              <table className="min-w-full divide-y divide-gray-200 mt-4">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {branchMigrationData.map((migration, index) => {
                    ...
                    const primaryReason = reasons.reduce(
              ...
              <div className="mt-4 space-y-4">
                <div className="bg-blue-50 p-4 rounded-md">
                  ...
                  <Text className="text-sm text-gray-600">
                    Relocation/moving is the dominant reason for branch changes,
                    accounting for{" "}
                    {Math.round(
                      (migrationReasonsSummary.find(
                        (r) => r.reason === "Relocated/Moved",
                      ).count /
                        migrationReasonsSummary.reduce(
                ...
                <div className="bg-green-50 p-4 rounded-md">
                  ...
                  <Text className="text-sm text-gray-600">
                    Members moving to Main Campus often cite program offerings
                    and service style preferences as key motivators, suggesting
                    stronger central programs may help with retention at smaller
                    branches.
                  </Text>
                </div>
                ...
                <div className="bg-amber-50 p-4 rounded-md">
                  ...
                  <Text className="text-sm text-gray-600">
                    The "Closer to Home" factor represents{" "}
                    {Math.round(
                      (migrationReasonsSummary.find(
                        (r) => r.reason === "Closer to Home",
                      ).count /
                        migrationReasonsSummary.reduce(
                          (sum, item) => sum + item.count,
                          0,
                    ...
                    % of migrations, suggesting that establishing more locations
                    in growing residential areas could benefit membership
                    retention.
                  </Text>
                </div>
              </div>
