...
import { Card, Title, Text, Grid, Select, SelectItem, Button } from '@tremor/react';

// Widget types
const WIDGET_TYPES = [
  { id: 'membership_growth', name: 'Membership Growth Chart', category: 'membership' },
  { id: 'attendance_trends', name: 'Attendance Trends', category: 'attendance' },
  { id: 'giving_summary', name: 'Giving Summary', category: 'financial' },
  { id: 'new_members', name: 'New Members', category: 'membership' },
  { id: 'volunteer_participation', name: 'Volunteer Participation', category: 'volunteer' },
  { id: 'ministry_engagement', name: 'Ministry Engagement', category: 'ministry' },
  { id: 'branch_comparison', name: 'Branch Comparison', category: 'branch' },
  { id: 'member_migration', name: 'Member Migration', category: 'membership' },
  { id: 'financial_overview', name: 'Financial Overview', category: 'financial' },
  { id: 'attendance_summary', name: 'Attendance Summary', category: 'attendance' },
  { id: 'sacramental_records', name: 'Sacramental Records', category: 'sacraments' },
  { id: 'branch_performance', name: 'Branch Performance', category: 'branch' }
];

// Pre-defined dashboard layouts
const DASHBOARD_PRESETS = [
  { id: 'leadership', name: 'Leadership Overview', 
    widgets: ['membership_growth', 'financial_overview', 'branch_comparison', 'volunteer_participation', 'member_migration'] },
  { id: 'branch_admin', name: 'Branch Administration', 
    widgets: ['attendance_summary', 'giving_summary', 'ministry_engagement', 'new_members'] },
  { id: 'finance', name: 'Financial Focus', 
    widgets: ['financial_overview', 'giving_summary', 'branch_comparison'] },
  { id: 'membership', name: 'Membership Focus', 
    widgets: ['membership_growth', 'new_members', 'member_migration', 'sacramental_records'] },
];
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-3">
              ...
              <div>
                ...
                <div className="max-h-64 overflow-auto border border-gray-200 rounded-md divide-y">
                  {availableWidgets.map((widget) => (
                    <div key={widget.id} className="p-3 flex justify-between items-center hover:bg-gray-50">
                      <div>
                        <Text className="font-medium">{widget.name}</Text>
                        <Text className="text-xs text-gray-500">Category: {widget.category}</Text>
                      </div>
                      <button
              ...
              <div className="mt-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center">
                    ...
                    />
                    <label htmlFor="all-access" className="ml-2 block text-sm text-gray-700">
                  ...
                  <div className="flex items-center">
                    <input 
                      ...
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                    />
                    <label htmlFor="branch-access" className="ml-2 block text-sm text-gray-700">
                  ...
                  <div className="flex items-center">
                    <input 
                      ...
                      className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300" 
                    />
                    <label htmlFor="limited-access" className="ml-2 block text-sm text-gray-700">
                      Home branch only
                    </label>
                  </div>
                </div>
        ...
        <div className="lg:col-span-2">
            ...
            <div className="mt-6">
              {activeWidgets.length === 0 ? (
                <div className="py-12 flex flex-col items-center justify-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-md">
                  ...
                  <Text className="mt-2 text-gray-500">No widgets selected</Text>
                  <Text className="text-sm text-gray-400">Add widgets from the configuration panel</Text>
                </div>
              ) : (
                <div className={`grid ${selectedLayout === 'list' ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-2'} gap-4`}>
                  {activeWidgets.map((widgetId) => (
                    <div 
                      key={widgetId} 
                      className={`bg-gray-50 border border-gray-200 rounded-md p-4 ${
                        widgetId === 'membership_growth' || widgetId === 'financial_overview' ? 'lg:col-span-2' : ''
                      }`}
                    >
                      <div className="flex justify-between items-center mb-2">
                        <Text className="font-medium">{getWidgetName(widgetId)}</Text>
                        <button
                      ...
                      <div className="h-32 flex items-center justify-center border border-dashed border-gray-300 rounded bg-white">
                        <Text className="text-sm text-gray-400">Widget Preview</Text>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <Card className="mt-6">
            ...
            <Text className="mt-2 text-gray-600">Share your dashboard configuration with other staff members</Text>
            
            <div className="mt-4 space-y-4">
              <div>
                <label htmlFor="share-with" className="block text-sm font-medium text-gray-700">
                  Share with Role
                </label>
              ...
              <div>
                <label htmlFor="share-branches" className="block text-sm font-medium text-gray-700">
                  Target Branches
                </label>
                <Select defaultValue="all">
                  <SelectItem value="all">All Branches</SelectItem>
