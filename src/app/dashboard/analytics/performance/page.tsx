...
import { Card, Title, Text, Grid, Col, Select, SelectItem, Flex } from '@tremor/react';
...
export default function BranchPerformancePage() {
  ...
  const selectedBranchData = branchMetrics.find(b => b.id === selectedBranch) || branchMetrics[0];
    ...
    <div className="p-6">
      ...
      <div className="mt-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium text-gray-900">Branch Performance Cards</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {branchMetrics.map((branch) => (
            <Card key={branch.id} className="hover:shadow-lg transition-shadow duration-200">
              <div className="flex justify-between items-center mb-2">
                <Title>{branch.name}</Title>
                <div className={`px-2 py-1 rounded text-xs font-medium 
                  ${branch.id === 'east' || branch.id === 'south' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}
                >
                  {branch.id === 'east' || branch.id === 'south' ? 'High Growth' : 'Stable'}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div>
                  ...
                  <div className="flex items-baseline gap-1 mt-1">
                    <span className="text-lg font-semibold">{branch.metrics.attendance.value}</span>
                    <span className="text-xs text-green-600">{branch.metrics.attendance.trend}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-1 mt-1">
                    <span className="text-lg font-semibold">{branch.metrics.giving.value}</span>
                    <span className="text-xs text-green-600">{branch.metrics.giving.trend}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-1 mt-1">
                    <span className="text-lg font-semibold">{branch.metrics.newMembers.value}</span>
                    <span className="text-xs text-green-600">{branch.metrics.newMembers.trend}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-1 mt-1">
                    <span className="text-lg font-semibold">{branch.metrics.volunteerRate.value}</span>
                    <span className="text-xs text-green-600">{branch.metrics.volunteerRate.trend}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-gray-200">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex items-baseline gap-1 mt-1">
                      <span className="text-lg font-semibold">{branch.metrics.discipleshipGroups.value}</span>
                      <span className="text-xs text-green-600">{branch.metrics.discipleshipGroups.trend}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-baseline gap-1 mt-1">
                      <span className="text-lg font-semibold">{branch.metrics.engagement.value}</span>
                      <span className="text-xs text-green-600">{branch.metrics.engagement.trend}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-4 space-y-4">
            <div className="bg-blue-50 p-4 rounded-md">
              ...
              <Text className="text-sm text-gray-600">
                Smaller branches (South Chapel, East Side) show higher engagement rates than larger branches, 
                demonstrating the effectiveness of intimate community connections.
              </Text>
            </div>
            ...
            <div className="bg-green-50 p-4 rounded-md">
              ...
              <Text className="text-sm text-gray-600">
                South Chapel is experiencing the fastest percentage growth across multiple metrics, 
                including new membership (+26.8%) and volunteer participation (+8.2%).
              </Text>
            </div>
            
            <div className="bg-amber-50 p-4 rounded-md">
              <Text className="font-medium">Resource Allocation Recommendation</Text>
              <Text className="text-sm text-gray-600">
                West End shows the lowest growth rates across most metrics. Consider implementing best practices 
                from South Chapel and East Side, particularly in member engagement and volunteer recruitment.
              </Text>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-md">
              ...
              <Text className="text-sm text-gray-600">
                The church network is experiencing healthy growth across all branches (average +7.4% in attendance), 
                with smaller branches growing faster but Main Campus continuing to provide stability and leadership.
              </Text>
            </div>
          </div>
        </Card>
