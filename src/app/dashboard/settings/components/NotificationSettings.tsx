...
export default function NotificationSettings() {
      ...
      setChannels((prev) =>
        prev.map((channel) => {
          const setting = data.settings.find(
            (s) => s.key === `notif_channel_${channel.id}`,
          );
          return setting
            ? { ...channel, enabled: setting.value === "true" }
            : channel;
        }),
      );
      setCategories((prev) =>
        prev.map((category) => {
          ...
          for (const ch of Object.keys(updatedChannels)) {
            const setting = data.settings.find(
              (s) => s.key === `notif_cat_${category.id}_${ch}`,
            );
  ...
  const toggleCategoryChannel = (
    categoryId: string,
    channelId: keyof NotificationCategory["channels"],
  ) => {
    setCategories((prev) =>
      prev.map((category) =>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updates = [
        ...channels.map((ch) => ({
          key: `notif_channel_${ch.id}`,
          value: String(ch.enabled),
        })),
        ...categories.flatMap((cat) =>
          Object.entries(cat.channels).map(([ch, enabled]) => ({
            key: `notif_cat_${cat.id}_${ch}`,
      ...
      const changedUpdates = updates.filter((u) => {
        ...
        return (
          !currentSetting || String(currentSetting.value) !== String(u.value)
        );
      });
      await Promise.all(
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        <div className="pt-6">
          ...
          <p className="mt-1 text-xs text-gray-500">
            Choose which types of notifications you want to receive and through
            which channels
          </p>
                  ...
                  {channels.map((channel) => (
                    ...
                    >
                      <input
                        ...
                        checked={
                          category.channels[
                            channel.id as keyof NotificationCategory["channels"]
                          ]
                        }
                        onChange={() =>
