      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-3 flex-1">
          <div className="text-2xl">
            {getRequestTypeIcon(session.requestType)}
          </div>
          <div className="flex-1">
            ...
            {(session.assignedPastor || session.assistantId) && (
              <div className="flex items-center text-sm text-gray-600 mt-1">
                ...
                {session.assignedPastor && (
                  <span>
                    Assigned Pastor: {session.assignedPastor.firstName}{" "}
                    {session.assignedPastor.lastName}
                  </span>
                )}
                {session.assignedPastor && session.assistantId && (
...
export default function CounselingSessionsList({
  sessions,
  loading,
  onCreateSession,
  onUpdateSession,
}: CounselingSessionsListProps) {
  const [isCreateFormOpen, setIsCreateFormOpen] = useState(false);

  // New state for advanced features
  const [editingSession, setEditingSession] = useState<CareRequest | null>(
    null,
  );
  const [selectedSession, setSelectedSession] = useState<CareRequest | null>(
    null,
  );
  ...
  const applyFilters = (sessions: CareRequest[]) => {
    return sessions.filter((session) => {
      ...
      if (
        ...
        !filters.type.includes(session.requestType)
      ) {
        return false;
      }
      ...
      if (
        ...
        !filters.priority.includes(session.priority)
      ) {
        return false;
      }
