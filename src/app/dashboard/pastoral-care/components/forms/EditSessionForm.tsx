...
const editSessionSchema = z.object({
  ...
  sessionType: z.enum(['INDIVIDUAL', 'COUPLE', 'FAMILY', 'GROUP', 'CRISIS', 'FOLLOW_UP']),
...
export default function EditSessionForm({ session, onClose, onUpdateSession }: EditSessionFormProps) {
  ...
  useEffect(() => {
    if (session) {
      ...
      setValue('scheduledDate', new Date(session.scheduledDate).toISOString().slice(0, 16));
              ...
              <div>
                ...
                {selectedStatus && (
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 ${
                    sessionStatuses.find(s => s.value === selectedStatus)?.color || 'bg-gray-100 text-gray-800'
                  }`}>
                    {sessionStatuses.find(s => s.value === selectedStatus)?.label}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-500"
                />
                <p className="text-xs text-gray-500 mt-1">Counselor assignment cannot be changed</p>
              </div>
            </div>
            ...
            {isConfidential && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
                <p className="text-sm text-yellow-800">
                  ...
                  This session will be marked as confidential and access will be restricted.
                </p>
              </div>
            )}
