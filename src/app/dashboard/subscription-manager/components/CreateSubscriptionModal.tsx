...
import { GET_SUBSCRIPTION_PLANS, GET_SUBSCRIPTION_ORGANIZATIONS } from '@/graphql/subscription-management';
...
export default function CreateSubscriptionModal({ isOpen, onClose, onSuccess }: CreateSubscriptionModalProps) {
  ...
  // GraphQL queries
  const { data: organizationsData, loading: organizationsLoading } = useQuery(GET_SUBSCRIPTION_ORGANIZATIONS, {
    skip: !isOpen,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ submit: error.message || 'Failed to create subscription. Please try again.' });
    } finally {
      setIsLoading(false);
    }
  };

  const selectedOrganization = organizations.find((org: any) => org.id === formData.organizationId);
  const selectedPlan = subscriptionPlans.find((plan: any) => plan.id === formData.planId);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {selectedOrganization && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <h4 className="font-medium text-blue-900 mb-2">Selected Organization</h4>
                      <div className="text-sm text-blue-700">
                        <p><strong>Name:</strong> {selectedOrganization.name}</p>
                        <p><strong>Email:</strong> {selectedOrganization.email}</p>
                        <p><strong>Status:</strong> <Badge variant="outline">{selectedOrganization.status}</Badge></p>
                      </div>
                    </div>
                  )}
                              ...
                              <div>
                                ...
                                <div className="flex items-center mt-2 space-x-4">
                                  ...
                                  {plan.trialPeriodDays > 0 && (
                                    <Badge variant="secondary">{plan.trialPeriodDays} day trial</Badge>
                                  )}
                                </div>
                              </div>
                              {formData.planId === plan.id && (
                  ...
                  {selectedOrganization && selectedPlan && (
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                      ...
                      <div className="text-sm text-gray-700 space-y-1">
                        ...
                        <p><strong>Amount:</strong> {formatCurrency(selectedPlan.amount, selectedPlan.currency)} / {selectedPlan.interval.toLowerCase()}</p>
                  ...
                  <div className="flex justify-end space-x-3 pt-4 border-t">
                    ...
                    <Button
                      type="submit"
                      disabled={isLoading || !formData.organizationId || !formData.planId}
                    >
