...
import TeamScheduling, {
  mockTeamMembers,
  mockSchedules,
  TeamMemberProfile,
} from "./components/TeamScheduling";
...
import RehearsalTracker, {
  mockRehearsals,
  Rehearsal,
  RehearsalAttendee,
} from "./components/features/RehearsalTracker";
...
export default function WorshipPage() {
  ...
  const completedServices = mockServices.filter(
    (service) => service.completed,
  ).length;
  ...
  const [showTeamMemberDetailsModal, setShowTeamMemberDetailsModal] =
    useState(false);
  const [showResourceDetailsModal, setShowResourceDetailsModal] =
    useState(false);
  const [showCreateServiceModal, setShowCreateServiceModal] = useState(false);
  const [showRehearsalDetailsModal, setShowRehearsalDetailsModal] =
    useState(false);
  const [showCreateRehearsalModal, setShowCreateRehearsalModal] =
    useState(false);

  // State for selected items
  const [selectedService, setSelectedService] = useState<ServicePlan | null>(
    null,
  );
  ...
  const handleSaveRehearsal = (
    ...
    if ("id" in rehearsalData) {
      ...
      const newRehearsal: Rehearsal = {
        ...
        id:
          rehearsals.length > 0
            ? Math.max(...rehearsals.map((r) => r.id)) + 1
            : 1,
      };
