...
export default function TeamMembersPage() {
  ...
  const allRoles = [
    "All Roles",
    ...new Set(mockTeamMembers.map((member) => member.role)),
  ].sort();
            ...
            >
              <div className="bg-indigo-50 px-6 py-4 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {membersByRole[role].length} members
                </p>
              </div>
                ...
                {membersByRole[role].map((member) => (
                  <li key={member.id} className="group">
                    <div
                      ...
                      onClick={() =>
                        router.push(`/dashboard/worship/members/${member.id}`)
                      }
                    >
                      ...
                      <div className="flex-1 min-w-0 sm:flex sm:items-center sm:justify-between">
                        <div>
                          <div className="flex items-center">
                            ...
                            <span
                              className={`ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(member.status)}`}
                            >
                              {member.status}
                            </span>
                          </div>
