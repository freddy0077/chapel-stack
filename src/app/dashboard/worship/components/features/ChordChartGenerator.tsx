...
export default function ChordChartGenerator({
  song,
}: ChordChartGeneratorProps) {
  ...
  const parseChordChartSections = useCallback(
    (chordChart: string): ChartSection[] => {
      ...
      lines.forEach((line) => {
        ...
        const sectionMatch = line.match(
          /^(verse|chorus|bridge|intro|outro)\s*(?:(\d+))?:?\s*$/i,
        );
  ...
  const extractUniqueChords = useCallback((chordChart: string): string[] => {
    const chordRegex =
      /\b[A-G][#b]?(?:m|maj|min|aug|dim|sus[24]?|[2-9]|7|13)?\b/g;
  ...
  const transposeChordChart = useCallback(
    (chordChart: string, fromKey: string, toKey: string): string => {
      ...
      const fromIndex = KEYS.findIndex((k) =>
        k.includes(fromKey.replace("m", "")),
      );
  ...
  const handleKeyChange = useCallback(
    (newKey: string) => {
      ...
      if (song.chordChart && song.defaultKey) {
        const transposed = transposeChordChart(
          song.chordChart,
          song.defaultKey,
          newKey,
        );
        ...
        <div className="flex flex-wrap gap-4">
          {uniqueChords.map((chord, index) => (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: generateChordDiagram(chord) }}
            />
          ))}
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <button
              ...
              onClick={() => {
                ...
                const blob = new Blob([transposedChart], {
                  type: "text/plain",
                });
