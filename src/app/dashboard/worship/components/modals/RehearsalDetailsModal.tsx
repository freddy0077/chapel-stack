...
interface RehearsalDetailsModalProps {
  ...
  onUpdateAttendance?: (rehearsalId: number, attendance: RehearsalAttendee[]) => void;
...
export default function RehearsalDetailsModal({ 
  ...
  const [attendanceData, setAttendanceData] = useState<RehearsalAttendee[]>(rehearsal.teamMembers);
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <p className="text-sm text-gray-500">{linkedService?.title || 'Independent Rehearsal'}</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-0 flex sm:mt-0">
                      ...
                      >
                        {rehearsal.completed ? 'Mark Incomplete' : 'Mark Complete'}
                      </button>
                      ...
                      <Tab.Panels className="mt-4">
                          ...
                          <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                              ...
                              <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
                                <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                  <div className="sm:col-span-1">
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900">{formatDate(rehearsal.date)}</dd>
                                  </div>
                                  <div className="sm:col-span-1">
                                    <dt className="text-sm font-medium text-gray-500 flex items-center">
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900">
                                      {rehearsal.time} ({formatDuration(rehearsal.duration)})
                                    </dd>
                                  </div>
                                  <div className="sm:col-span-1">
                                    <dt className="text-sm font-medium text-gray-500 flex items-center">
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900">{rehearsal.location}</dd>
                                  </div>
                                  <div className="sm:col-span-1">
                                    <dt className="text-sm font-medium text-gray-500 flex items-center">
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900">{rehearsal.teamMembers.length}</dd>
                                  </div>
                                  {linkedService && (
                                    <div className="sm:col-span-2">
                                      <dt className="text-sm font-medium text-gray-500">For Service</dt>
                                      <dd className="mt-1 text-sm text-gray-900">
                                        {linkedService.title} ({new Date(linkedService.date).toLocaleDateString()})
                                      </dd>
                                    </div>
                                  )}
                                </dl>
                            ...
                            <div className="sm:col-span-3">
                              <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
                                ...
                                <dt className="text-sm font-medium text-gray-500 mt-4">Notes</dt>
                                <dd className="mt-1 text-sm text-gray-900 whitespace-pre-wrap">
                                        ...
                                        <tr key={member.teamMemberId}>
                                          ...
                                          <td className="border-b border-gray-200 px-3 py-4 text-sm text-gray-500">
                                            <input
                                              type="text"
                                              value={member.notes}
                                              onChange={(e) => updateNote(member.teamMemberId, e.target.value)}
                                          ...
                                          {isRehearsalPast() && (
                                            <td className="border-b border-gray-200 py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 lg:pr-8">
                                              <div className="flex space-x-2">
                                                <button
                                                  type="button"
                                                  onClick={() => updateAttendance(member.teamMemberId, true)}
                                                  className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${
                                                    member.attended === true
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() => updateAttendance(member.teamMemberId, false)}
                                                  className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${
                                                    member.attended === false
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() => updateAttendance(member.teamMemberId, null)}
                                                  className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${
                        ...
                        <Tab.Panel>
                          <div className="px-4 py-5 sm:px-6">
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                              {rehearsal.songs.length} songs planned for this rehearsal
                            </p>
                            ...
                            {rehearsal.songs.length > 0 ? (
                                  ...
                                  <li key={idx} className="flex items-center justify-between gap-x-6 py-5">
                                    <div className="flex min-w-0 gap-x-4">
                                      ...
                                      <div className="min-w-0 flex-auto">
                                        <p className="text-sm font-semibold leading-6 text-gray-900">{song}</p>
                                        <p className="mt-1 truncate text-xs leading-5 text-gray-500">
                                          {linkedService?.songs.find(s => s.title === song)?.key || ''}
                              ...
                              <div className="mt-4 text-center py-12 px-4">
                                ...
                                <h3 className="mt-2 text-sm font-semibold text-gray-900">No songs specified</h3>
                                <p className="mt-1 text-sm text-gray-500">
                                  No specific songs have been listed for this rehearsal.
                                </p>
                              </div>
                            )}
                          </div>
