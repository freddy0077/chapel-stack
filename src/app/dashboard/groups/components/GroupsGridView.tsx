...
export default function GroupsGridView({
  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {groups.map((group) => {
        const { badgeColor, textColor, borderColor } = getStatusPresentation(
          group.status as SmallGroupStatus,
        );
            ...
            <div className="p-5 flex flex-col flex-grow">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`h-12 w-12 rounded-full flex items-center justify-center bg-indigo-50 ${textColor}`}
                  >
                ...
                <div className="flex-shrink-0 relative">
                  ...
                  >
                    {group.status === SmallGroupStatus.ACTIVE
                      ? "Active"
                      : group.status === SmallGroupStatus.INACTIVE
                        ? "Inactive"
                        : "Archived"}
                  </span>
                </div>
