...
const mockMembers: Record<string, Attendee[]> = {
  '1': [
    { id: '101', name: 'Alex Johnson', email: 'alex@example.com', phone: '555-1234', status: 'present' },
    { id: '102', name: 'Taylor Swift', email: 'taylor@example.com', phone: '555-2345', status: 'present' },
    { id: '103', name: 'Jordan Lee', email: 'jordan@example.com', phone: '555-3456', status: 'absent' },
    { id: '104', name: 'Morgan Chen', email: 'morgan@example.com', phone: '555-4567', status: 'present' },
    { id: '105', name: 'Casey Kim', email: 'casey@example.com', phone: '555-5678', status: 'excused', notes: 'Out of town' },
    { id: '106', name: 'Riley Singh', email: 'riley@example.com', phone: '555-6789', status: 'present' },
    { id: '107', name: 'Quinn Wu', email: 'quinn@example.com', phone: '555-7890', status: 'late', notes: 'Work' },
    { id: '108', name: 'Jamie Patel', email: 'jamie@example.com', phone: '555-8901', status: 'present' },
  ],
  '2': [
    { id: '201', name: 'Dana Cruz', email: 'dana@example.com', phone: '555-1111', status: 'present' },
    { id: '202', name: 'Sam Rodriguez', email: 'sam@example.com', phone: '555-2222', status: 'present' },
    { id: '203', name: 'Jesse Morgan', email: 'jesse@example.com', phone: '555-3333', status: 'absent' },
  ],
  '3': [
    { id: '301', name: 'Avery Wilson', email: 'avery@example.com', phone: '555-4444', status: 'present' },
    { id: '302', name: 'Blake Thompson', email: 'blake@example.com', phone: '555-5555', status: 'present' },
    { id: '303', name: 'Cameron Davis', email: 'cameron@example.com', phone: '555-6666', status: 'excused' },
  ],
};

// Mock attendance data
const generateAttendanceDates = (groupId: string, count: number): MeetingAttendance[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const totalPresent = attendees.filter(a => a.status === 'present' || a.status === 'late').length;
...
function generateRandomAttendanceStatus(): 'present' | 'absent' | 'late' | 'excused' | 'unknown' {
...
const generateAttendanceTrends = (groupId: string) => {
  ...
  return data.slice(0, 8).map(record => ({
...
export default function GroupAttendance() {
  ...
  const groupAttendance = selectedGroup ? (mockAttendance[selectedGroup] || []) : [];
  
  // Set default attendance data when group changes
  useEffect(() => {
    if (selectedGroup && groupAttendance.length > 0) {
      const meetingRecord = selectedDate === 'latest' 
        ? groupAttendance[0] 
        : groupAttendance.find(record => record.date.toISOString().split('T')[0] === selectedDate);
  ...
  const updateAttendanceStatus = (attendeeId: string, status: 'present' | 'absent' | 'late' | 'excused' | 'unknown') => {
  ...
  const trendData = selectedGroup ? generateAttendanceTrends(selectedGroup) : [];
            ...
            {selectedGroup ? (
              <div className="bg-white shadow rounded-lg">
                <div className="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between items-center">
                  <div>
                    <h3 className="text-lg leading-6 font-medium text-gray-900">
                      {mockGroups.find(g => g.id === selectedGroup)?.name || 'Selected Group'}
                    </h3>
                    <p className="mt-1 text-sm text-gray-500">
                      Leader: {mockGroups.find(g => g.id === selectedGroup)?.leader}
                    </p>
                  </div>
                ...
                <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                  <div className="flex space-x-2">
                    ...
                    <Badge color="amber">
                      {attendanceData.filter(a => a.status === 'late').length} late
                    </Badge>
                    <Badge color="red">
                      {attendanceData.filter(a => a.status === 'absent').length} absent
                    </Badge>
                    <Badge color="blue">
                      {attendanceData.filter(a => a.status === 'excused').length} excused
                    </Badge>
                  </div>
                  <div className="flex space-x-2">
                    {editMode ? (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              {attendee.status === 'present' && (
                                <Badge color="emerald" icon={CheckCircleIcon}>Present</Badge>
                              )}
                              {attendee.status === 'absent' && (
                                <Badge color="red" icon={XCircleIcon}>Absent</Badge>
                              )}
                              ...
                              {attendee.status === 'unknown' && (
                                <Badge color="gray" icon={QuestionMarkCircleIcon}>Unknown</Badge>
                              )}
                              {attendee.notes && (
                          ...
                          {editMode && (
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                              <div className="flex justify-end space-x-2">
                                <button
                                  onClick={() => updateAttendanceStatus(attendee.id, 'present')}
