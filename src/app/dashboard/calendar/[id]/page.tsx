...
export default function EventDetail() {
  ...
  const getEventTimeRange = (startDateTime: string | Date | null | undefined, endDateTime: string | Date | null | undefined) => {
  ...
  const getEventTypeColor = (type: string) => {
    switch(type) {
      ...
      case EventType.MEETING: return "amber";
      case EventType.CONFERENCE: return "purple";
      case EventType.WORKSHOP: return "emerald";
      case EventType.RETREAT: return "indigo";
      case EventType.OUTREACH: return "rose";
      case EventType.SOCIAL: return "pink";
      default: return "gray";
    }
  };
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <div className="bg-red-50 p-4 rounded-lg">
          ...
          <div className="mt-4">
            <Link href="/dashboard/calendar" className="inline-flex items-center px-4 py-2 rounded-md bg-white text-red-700 border border-red-300 text-sm font-medium hover:bg-red-50 transition-colors">
    ...
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <div className="bg-yellow-50 p-4 rounded-lg">
          ...
          <p className="mt-2 text-sm text-yellow-700">The event you&apos;re looking for doesn&apos;t exist or has been removed.</p>
          <div className="mt-4">
            <Link href="/dashboard/calendar" className="inline-flex items-center px-4 py-2 rounded-md bg-white text-yellow-700 border border-yellow-300 text-sm font-medium hover:bg-yellow-50 transition-colors">
              <ArrowLeftIcon className="h-4 w-4 mr-2" />
              Back to Calendar
            </Link>
          </div>
    ...
    <>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="sticky top-0 z-10 bg-white bg-opacity-90 backdrop-blur-md border-b border-gray-100 -mx-6 px-6 py-4 mb-8 flex justify-between items-center">
          <Link href="/dashboard/calendar" className="flex items-center group transition-all duration-300">
        ...
        <div className="relative mb-8 overflow-hidden rounded-3xl shadow-xl">
          ...
          <div className="relative px-8 py-12 text-white">
            <div className="flex flex-wrap gap-3 mb-4">
              <div className={`px-3 py-1 rounded-full ${getStatusColor(event.status) === "green" ? "bg-green-500" : 
                getStatusColor(event.status) === "yellow" ? "bg-yellow-500" : 
                getStatusColor(event.status) === "red" ? "bg-red-500" : 
                getStatusColor(event.status) === "blue" ? "bg-blue-500" : "bg-gray-500"} 
                bg-opacity-30 backdrop-blur-sm text-xs font-medium border border-white border-opacity-20`}>
                {event.status}
              </div>
              <div className={`px-3 py-1 rounded-full ${getEventTypeColor(event.type) === "blue" ? "bg-blue-500" : 
                getEventTypeColor(event.type) === "purple" ? "bg-purple-500" : 
                getEventTypeColor(event.type) === "amber" ? "bg-amber-500" : 
                getEventTypeColor(event.type) === "emerald" ? "bg-emerald-500" : 
                getEventTypeColor(event.type) === "indigo" ? "bg-indigo-500" : 
                getEventTypeColor(event.type) === "rose" ? "bg-rose-500" : 
                getEventTypeColor(event.type) === "pink" ? "bg-pink-500" : "bg-gray-500"} 
                bg-opacity-30 backdrop-blur-sm text-xs font-medium border border-white border-opacity-20`}>
                {event.type}
              </div>
            </div>
            ...
            {event.description && (
              <p className="text-white text-opacity-90 max-w-2xl text-lg">{event.description}</p>
            )}
          </div>
        </div>
        {/* Event Details */}
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="rounded-xl bg-gray-50/70 p-4 flex items-center hover:shadow-md transition-all duration-300">
                    ...
                    <div>
                      <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Date</div>
                  ...
                  <div className="rounded-xl bg-gray-50/70 p-4 flex items-center hover:shadow-md transition-all duration-300">
                    ...
                    <div>
                      <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Time</div>
                      <div className="font-medium text-gray-900">{getEventTimeRange(event.startDate, event.endDate)}</div>
                    </div>
                  </div>
                  <div className="rounded-xl bg-gray-50/70 p-4 flex items-center hover:shadow-md transition-all duration-300">
                    ...
                    <div>
                      <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Location</div>
                      <div className="font-medium text-gray-900">{event.location || "No location specified"}</div>
                    </div>
                  </div>
                  {event.capacity && (
                    <div className="rounded-xl bg-gray-50/70 p-4 flex items-center hover:shadow-md transition-all duration-300">
                      ...
                      <div>
                        <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Capacity</div>
                        <div className="font-medium text-gray-900">{event.capacity} people</div>
                      </div>
                    </div>
                  )}
                  {event.branch && (
                    <div className="rounded-xl bg-gray-50/70 p-4 flex items-center hover:shadow-md transition-all duration-300">
                      ...
                      <div>
                        <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Branch</div>
                        <div className="font-medium text-gray-900">{event.branch.name}</div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="p-6">
              <div className="space-y-5">
                <div className="bg-gray-50/70 p-4 rounded-xl">
                  <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Registration Required</div>
                ...
                {event.registrationRequired && event.registrationDeadline && (
                  <div className="bg-gray-50/70 p-4 rounded-xl">
                    <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Registration Deadline</div>
                    <div className="font-medium text-gray-900">{formatEventDate(event.registrationDeadline)}</div>
                  </div>
                )}
                {event.isRecurring && (
                  <div className="bg-gray-50/70 p-4 rounded-xl">
                    <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Recurring Event</div>
                    <div className="font-medium text-gray-900">Yes - {event.recurrencePattern}</div>
                  </div>
                )}
                <Divider />
                <div className="p-4 rounded-xl">
                  <div className="text-xs uppercase tracking-wider text-gray-500 font-medium mb-1">Last Updated</div>
                  <div className="font-medium text-gray-900">{formatEventDate(event.updatedAt)}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100">
              <div className="px-6 py-5 border-b border-gray-100 bg-gray-50/60 flex justify-between items-center">
                ...
                <div className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm font-medium">
                  {event.registrations.length} {event.registrations.length === 1 ? 'person' : 'people'}
                </div>
              </div>
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="bg-gray-50/80 border-b border-gray-100">
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Member</th>
                      ...
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Check In</th>
                      <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Check Out</th>
                    </tr>
                  </thead>
                    ...
                    {event.registrations.map((registration: EventRegistration) => (
                      <tr key={registration.id} className="hover:bg-gray-50/50 transition-colors duration-200">
                        <td className="px-6 py-5 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-gray-500">{registration.member.email}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-5 whitespace-nowrap">
                        ...
                        <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-500">
                          {registration.registrationDate ? formatEventDate(registration.registrationDate) : "-"}
                        </td>
                        <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-500">
                          {registration.checkInTime ? formatEventTime(registration.checkInTime) : "-"}
                        </td>
                        <td className="px-6 py-5 whitespace-nowrap text-sm text-gray-500">
                          {registration.checkOutTime ? formatEventTime(registration.checkOutTime) : "-"}
                        </td>
                      </tr>
                    ))}
                  </tbody>
