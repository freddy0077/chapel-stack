    ...
    <div className="bg-white shadow rounded-lg overflow-hidden">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {editingNotesFor === transaction.id ? (
                        ...
                        <div className="flex items-center group">
                          <span className="max-w-xs truncate mr-2">{transaction.notes || "No notes"}</span>
                          <button
                            type="button"
                            onClick={() => startEditingNotes(transaction.id, transaction.notes)}
                      ...
                      {transaction.isMatched ? (
                        <div>
                          <div className="text-xs text-gray-500 mb-1">
                            Matched to: {matchedChurchTx?.description || 'Unknown'}
                          </div>
                          <button
                            type="button"
                            onClick={() => handleUnmatch(transaction.id, transaction.matchedToId as string)}
      ...
      {selectedBankTx && (
        <div className="border-t border-gray-200 bg-gray-50 px-4 py-4 sm:px-6">
          <h3 className="text-sm font-medium text-gray-900 mb-3">Potential Matches</h3>
          
          {(() => {
            const selectedTransaction = bankTransactions.find(tx => tx.id === selectedBankTx);
                    ...
                    {potentialMatches.map((tx) => (
                      <tr key={tx.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button
                            type="button"
                            onClick={() => handleMatch(selectedBankTx as string, tx.id)}
