...
export default function Finances() {
  ...
  } = useFinanceDashboardData(branchId);
  ...
  } = useContributionsStats(branchId);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8">
        <div className="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-base text-gray-600">
                Comprehensive view of your church's financial health and
                activities
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/dashboard/finances/new-transaction"
              passHref
              legacyBehavior
            >
            ...
            <Link
              ...
              className="inline-flex items-center rounded-xl bg-white/80 px-4 py-2 text-sm font-semibold text-indigo-700 shadow hover:bg-indigo-50 border border-indigo-100 focus:outline-none focus:ring-2 focus:ring-indigo-400"
            >
              View Reports
            </Link>
          </div>
        ...
        <div className="mb-8">
          <div className="rounded-2xl bg-gradient-to-tr from-indigo-50/80 to-purple-50/80 shadow-xl p-6">
            <FinancialOverview
              totalDonations={
                contributionsStats
                  ? `$${contributionsStats.totalAmount?.toLocaleString()}`
                  : financialOverview.totalDonations
              }
              ...
              monthlyChange={
                contributionsStats &&
                contributionsStats.percentChangeFromPreviousPeriod !== undefined
                  ? `${contributionsStats.percentChangeFromPreviousPeriod > 0 ? "+" : ""}${contributionsStats.percentChangeFromPreviousPeriod}%`
                  : financialOverview.monthlyChange
              }
              donationTrends={
                contributionsStats?.trendData?.map((d) => d.amount) || []
              }
        ...
        <div className="mb-8">
          <div className="rounded-2xl bg-white/70 shadow-lg backdrop-blur-xl p-6">
            <h2 className="text-xl font-bold text-indigo-900 mb-4 flex items-center gap-2">
              <ArrowsUpDownIcon className="h-6 w-6 text-indigo-400" /> Recent
              Transactions
            </h2>
            <TransactionsTable
        ...
        <div className="mt-8 overflow-hidden rounded-2xl bg-white/70 shadow-xl backdrop-blur-xl">
          <div className="px-6 py-5">
            <h3 className="text-lg font-semibold leading-6 text-indigo-900 mb-4 flex items-center gap-2">
              <ChartBarIcon className="h-5 w-5 text-indigo-500" /> Financial
              Quick Actions
            </h3>
