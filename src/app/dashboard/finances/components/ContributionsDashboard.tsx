    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {widgets.map((w) => (
          <div
            key={w.label}
            className={`bg-gradient-to-br ${w.color} rounded-2xl shadow-xl p-6 flex flex-col items-start relative overflow-hidden backdrop-blur-xl`}
          >
      ...
      <div className="flex flex-wrap gap-4">
        {quickActions.map((a) => (
          <a
            key={a.label}
            href={a.href}
            className={`inline-flex items-center gap-2 rounded-xl bg-gradient-to-br ${a.color} px-5 py-3 text-sm font-semibold text-white shadow-lg hover:scale-[1.03] transition-transform`}
          >
            {a.icon}
            {a.label}
          </a>
