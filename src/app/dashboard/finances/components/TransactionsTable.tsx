...
const TransactionsTable = ({ transactions, branches }: TransactionsTableProps) => {
  ...
  const filteredTransactions = transactions.filter(transaction => {
    ...
    if (searchQuery) {
      ...
      return (
        ...
        (transaction.donor && transaction.donor.toLowerCase().includes(query)) ||
      ...
      <div className="p-6 border-b border-indigo-100">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="text-xl font-bold text-indigo-900 flex items-center gap-2">
            <ArrowsUpDownIcon className="h-5 w-5 text-indigo-400" /> Recent Transactions
          </h2>
          ...
          <form className="flex gap-3 flex-wrap items-end w-full sm:w-auto bg-gradient-to-br from-indigo-50/60 to-white/80 rounded-2xl px-4 py-3 shadow-lg border border-indigo-100 backdrop-blur-xl">
            ...
            <div className="flex flex-col w-full sm:w-56">
              <label className="text-xs font-semibold text-indigo-500 mb-1 ml-1" htmlFor="transaction-search">Search</label>
            ...
            <div className="flex flex-col w-full sm:w-auto">
              <label htmlFor="transaction-type-filter" className="text-xs font-semibold text-indigo-500 mb-1 ml-1">Type</label>
            ...
            <div className="flex flex-col w-full sm:w-44">
              <label htmlFor="transaction-branch-filter" className="text-xs font-semibold text-indigo-500 mb-1 ml-1">Branch</label>
              <select
        ...
        <table className="min-w-full divide-y divide-indigo-100">
            ...
            <tr>
              ...
              >
                <div className="flex items-center">
                  ...
                  {sortBy !== "date" && <ArrowsUpDownIcon className="ml-1 h-4 w-4 text-indigo-200" />}
                </div>
              </th>
              <th
                scope="col"
              ...
              >
                <div className="flex items-center">
                  ...
                  {sortBy !== "type" && <ArrowsUpDownIcon className="ml-1 h-4 w-4 text-indigo-200" />}
                </div>
              </th>
              <th
                scope="col"
              ...
              >
                <div className="flex items-center">
                  ...
                  {sortBy !== "category" && <ArrowsUpDownIcon className="ml-1 h-4 w-4 text-indigo-200" />}
                </div>
              </th>
              <th
                scope="col"
              ...
              >
                <div className="flex items-center">
                  ...
                  {sortBy !== "branchId" && <ArrowsUpDownIcon className="ml-1 h-4 w-4 text-indigo-200" />}
                </div>
              </th>
              <th
                scope="col"
              ...
              >
                <div className="flex items-center">
                  ...
                  {sortBy !== "amount" && <ArrowsUpDownIcon className="ml-1 h-4 w-4 text-indigo-200" />}
                </div>
              </th>
              <th
                scope="col"
          ...
          <tbody className="bg-white/70 divide-y divide-indigo-50">
            {filteredTransactions.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-10 text-center text-indigo-400">
                  No transactions found matching your criteria.
                </td>
              </tr>
            ) : (
              filteredTransactions.map((transaction) => (
                <tr key={transaction.id} className="hover:bg-indigo-50/60 transition-colors duration-150">
                  ...
                  <td className={`px-6 py-4 whitespace-nowrap text-sm font-extrabold ${transaction.type === "Income" ? "text-emerald-600" : "text-rose-600"}`}>
                    {transaction.amount}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-indigo-700">
