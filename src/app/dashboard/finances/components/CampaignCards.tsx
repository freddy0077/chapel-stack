...
const CampaignCards = ({ campaigns }: CampaignCardsProps) => {
  ...
  const getColorByProgress = (
    ...
    if (progress < 35)
      return { bg: "bg-indigo-100/70", text: "text-indigo-800" };
    if (progress < 70)
      return { bg: "bg-emerald-100/70", text: "text-emerald-800" };
        ...
        {campaigns.map((campaign) => (
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3 z-10 relative">
              <div
                className={`p-3 rounded-xl ${getColorByProgress(campaign.progress).bg} ${getColorByProgress(campaign.progress).text} shadow-md`}
              >
                <GiftIcon className="h-6 w-6" />
              </div>
              <span className="ml-3 text-base font-bold text-indigo-900 truncate max-w-[10rem]">
                {campaign.name}
              </span>
            </div>
