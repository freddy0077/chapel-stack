    ...
    <div className="bg-white/80 rounded-2xl shadow-xl p-6 mt-8">
      <div className="flex flex-wrap gap-3 items-end mb-6">
        <div className="flex flex-col">
          <label className="text-xs font-semibold text-rose-500 mb-1 ml-1">
            Date
          </label>
          <input
            type="date"
            className="rounded-xl border border-rose-100 px-3 py-2 bg-white/90 shadow focus:ring-2 focus:ring-rose-300"
            value={date}
        ...
        <div className="flex flex-col flex-1 min-w-[160px]">
          ...
          <div className="relative">
            <input
              type="text"
              className="rounded-xl border border-rose-100 px-3 py-2 w-full bg-white/90 shadow focus:ring-2 focus:ring-rose-300 pl-10"
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gradient-to-r from-rose-50 to-white">
              <th className="px-4 py-3 text-left font-bold text-rose-700">
                Date
              </th>
              <th className="px-4 py-3 text-left font-bold text-rose-700">
                Payee
              ...
              <th className="px-4 py-3 text-center font-bold text-rose-700">
                Receipt
              </th>
              <th className="px-4 py-3 text-center font-bold text-rose-700">
                Actions
          ...
          <tbody>
            {mockExpenses.map((e) => (
              <tr
                key={e.id}
                className="border-b border-rose-50 hover:bg-rose-50/40"
              >
                <td className="px-4 py-2">{e.date}</td>
                <td className="px-4 py-2">{e.payee}</td>
                <td className="px-4 py-2">{e.category}</td>
                <td className="px-4 py-2 font-bold text-rose-600">
                  {e.amount}
                </td>
                ...
                <td className="px-4 py-2 text-center">
                  {e.receipt && (
                    <PaperClipIcon className="h-5 w-5 text-rose-400 inline" />
                  )}
                </td>
                <td className="px-4 py-2 text-center">
