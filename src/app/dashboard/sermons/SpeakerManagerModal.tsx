    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">{initialData ? 'Edit Speaker' : 'Add Speaker'}</h2>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="speaker-title" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div>
            <label htmlFor="speaker-bio" className="block text-sm font-medium text-gray-700 mb-1">
              Biography (Optional)
            </label>
            <textarea
              id="speaker-bio"
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl">
        ...
        {!loading && speakers?.length === 0 && (
          <div className="text-center py-8 bg-gray-50 rounded-lg">
            <p className="text-gray-500">No speakers found. Create your first speaker!</p>
          </div>
        )}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {speakers.map((speaker) => (
                  <tr key={speaker.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap font-medium">{speaker.name}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {speaker.title || <span className="text-gray-400 italic">No title</span>}
                    </td>
