  ...
  growth: [
    {
      ...
      "Sacred Heart Parish": 21,
      "St. Joseph's Church": 16,
      "Holy Trinity Church": 12,
      "St. Peter's Parish": 14,
      "Good Shepherd Church": 10,
    },
    {
      month: "February",
      year: 2025,
      "St. Mary's Cathedral": 31,
      "Sacred Heart Parish": 23,
      "St. Joseph's Church": 17,
      "Holy Trinity Church": 13,
      "St. Peter's Parish": 15,
      "Good Shepherd Church": 11,
    },
    {
      month: "March",
      year: 2025,
      "St. Mary's Cathedral": 34,
      "Sacred Heart Parish": 25,
      "St. Joseph's Church": 19,
...
export default function MembershipReportSection({
  ...
  const filteredTotalsData = mockMembershipData.totals.filter(
    (item) =>
      selectedBranchNames.includes(item.branch) ||
      selectedBranchNames.length === 0,
  );

  const filteredDemographicsData = mockMembershipData.demographics.filter(
    (item) =>
      selectedBranchNames.includes(item.branch) ||
      selectedBranchNames.length === 0,
  );
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
                {calculateTotalMembers().toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
                {calculateActiveMembers().toLocaleString()}
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredTotalsData.map((branch, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {branch.active.toLocaleString()} (
                      {Math.round((branch.active / branch.total) * 100)}%)
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {branch.inactive.toLocaleString()} (
                      {Math.round((branch.inactive / branch.total) * 100)}%)
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      {branch.newMembers}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <span
                        className={`${branch.newMembers - branch.departed > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                      >
                        {branch.newMembers - branch.departed > 0 ? "+" : ""}
                        {branch.newMembers - branch.departed}
                      </span>
                    </td>
                  </tr>
                ))}
                ...
                <tr className="bg-gray-50 dark:bg-gray-750 font-medium">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredTotalsData
                      .reduce((sum, branch) => sum + branch.total, 0)
                      .toLocaleString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredTotalsData
                      .reduce((sum, branch) => sum + branch.active, 0)
                      .toLocaleString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredTotalsData
                      .reduce((sum, branch) => sum + branch.inactive, 0)
                      .toLocaleString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredTotalsData.reduce(
                      (sum, branch) => sum + branch.newMembers,
                      0,
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredTotalsData.reduce(
                      (sum, branch) => sum + branch.departed,
                      0,
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <span
                      className={`${filteredTotalsData.reduce((sum, branch) => sum + (branch.newMembers - branch.departed), 0) > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                    >
                      {filteredTotalsData.reduce(
                        (sum, branch) =>
                          sum + (branch.newMembers - branch.departed),
                        0,
                      ) > 0
                        ? "+"
                        : ""}
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredDemographicsData.map((branch, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        {branch.under18} (
                        {Math.round((branch.under18 / total) * 100)}%)
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        {branch.age18to35} (
                        {Math.round((branch.age18to35 / total) * 100)}%)
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        {branch.age36to55} (
                        {Math.round((branch.age36to55 / total) * 100)}%)
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        {branch.over55} (
                        {Math.round((branch.over55 / total) * 100)}%)
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                        {total}
                      </td>
                ...
                <tr className="bg-gray-50 dark:bg-gray-750 font-medium">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredDemographicsData.reduce(
                      (sum, branch) => sum + branch.under18,
                      0,
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredDemographicsData.reduce(
                      (sum, branch) => sum + branch.age18to35,
                      0,
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredDemographicsData.reduce(
                      (sum, branch) => sum + branch.age36to55,
                      0,
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                    {filteredDemographicsData.reduce(
                      (sum, branch) => sum + branch.over55,
                      0,
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                      {branchesToDisplay.reduce(
                        (total, branch) => total + (month[branch] || 0),
                        0,
                      )}
                    </td>
                  </tr>
                ...
                <tr className="bg-gray-50 dark:bg-gray-750 font-medium">
                  ...
                  {branchesToDisplay.map((branch) => (
                    ...
                    >
                      {filteredGrowthData.reduce(
                        (total, month) => total + (month[branch] || 0),
                        0,
                      )}
                    </td>
                  ))}
