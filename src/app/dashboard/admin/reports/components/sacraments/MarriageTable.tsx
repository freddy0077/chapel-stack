...
const upcomingAnniversaries = [
  {
    couple: "Robert & Maria Smith",
    branch: "St. Mary's Cathedral",
    date: "2020-04-15",
    years: 5,
  },
  {
    couple: "James & Elizabeth Johnson",
    branch: "St. Mary's Cathedral",
    date: "2015-05-10",
    years: 10,
  },
  {
    couple: "Michael & Catherine Brown",
    branch: "Sacred Heart Parish",
    date: "2015-05-23",
    years: 10,
  },
  {
    couple: "David & Jennifer Wilson",
    branch: "St. Joseph's Church",
    date: "2000-06-12",
    years: 25,
  },
  {
    couple: "Thomas & Susan Davis",
    branch: "Holy Trinity Church",
    date: "2020-04-30",
    years: 5,
  },
  {
    couple: "William & Patricia Martinez",
...
export default function MarriageTable({
  ...
  const calculateMonthTypeTotal = (
    month: unknown,
    type: "firstTime" | "convalidation" | "withMass" | "withoutMass",
  ) => {
    return branchesToDisplay.reduce((total, branch) => {
  ...
  const calculateBranchTypeTotal = (
    branch: string,
    type: "firstTime" | "convalidation" | "withMass" | "withoutMass",
  ) => {
    return mockMarriageData.reduce((total, month) => {
  ...
  const filteredAnniversaries = upcomingAnniversaries.filter(
    (anniversary) =>
      branchesToDisplay.includes(anniversary.branch) ||
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Records by month and marriage type
          </p>
        </div>
        ...
        <tfoot className="bg-gray-50 dark:bg-gray-750">
          <tr>
            ...
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce(
                (total, branch) =>
                  total + calculateBranchTypeTotal(branch, "firstTime"),
                0,
              )}
            </td>
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce(
                (total, branch) =>
                  total + calculateBranchTypeTotal(branch, "convalidation"),
                0,
              )}
            </td>
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce(
                (total, branch) =>
                  total + calculateBranchTypeTotal(branch, "withMass"),
                0,
              )}
            </td>
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce(
                (total, branch) =>
                  total + calculateBranchTypeTotal(branch, "withoutMass"),
                0,
              )}
            </td>
          </tr>
        ...
        <div>
          ...
          <div className="bg-pink-50 dark:bg-pink-900/20 rounded-lg p-4 text-sm text-pink-800 dark:text-pink-300">
            <ul className="list-disc pl-5 space-y-1">
              <li>
                First-time marriages account for{" "}
                {Math.round(
                  (branchesToDisplay.reduce(
                    (total, branch) =>
                      total + calculateBranchTypeTotal(branch, "firstTime"),
              ...
              <li>
                Marriage ceremonies increase by about 25% from January to March
              </li>
            </ul>
          </div>
        </div>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredAnniversaries.length > 0 ? (
                  filteredAnniversaries.map((anniversary, index) => (
                    <tr
                      key={`${anniversary.couple}-${anniversary.date}`}
                      className={
                        index % 2 === 0
                          ? "bg-white dark:bg-gray-800"
                          : "bg-gray-50 dark:bg-gray-750"
