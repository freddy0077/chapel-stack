...
const mockCommunionData = [
  { 
    month: 'January', 
    year: 2025,
    'St. Mary\'s Cathedral': { total: 8, ages7to10: 6, ages11to14: 2, adults: 0 },
    ...
    'Good Shepherd Church': { total: 3, ages7to10: 2, ages11to14: 1, adults: 0 }
  },
  { 
    month: 'February', 
    year: 2025,
    'St. Mary\'s Cathedral': { total: 9, ages7to10: 7, ages11to14: 1, adults: 1 },
    ...
    'Good Shepherd Church': { total: 2, ages7to10: 2, ages11to14: 0, adults: 0 }
  },
  { 
    month: 'March', 
    year: 2025,
    'St. Mary\'s Cathedral': { total: 12, ages7to10: 8, ages11to14: 3, adults: 1 },
    ...
    'Good Shepherd Church': { total: 4, ages7to10: 3, ages11to14: 1, adults: 0 }
  }
];
...
export default function CommunionTable({
  ...
  const calculateBranchAgeGroupTotal = (branch: string, ageGroup: 'ages7to10' | 'ages11to14' | 'adults') => {
    return mockCommunionData.reduce((total, month) => {
      return total + (month[branch]?.[ageGroup] || 0);
    }, 0);
  };
  ...
  return (
    <div className="p-4 overflow-x-auto">
      <div className="flex justify-between items-center mb-4">
        <div>
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">First Communion Statistics</h3>
      ...
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead className="bg-gray-50 dark:bg-gray-750">
          <tr>
            ...
            {branchesToDisplay.map(branch => (
              <th key={branch} scope="col" colSpan={4} className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                {branch}
              </th>
            ))}
            <th scope="col" colSpan={4} className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
              Total
            </th>
          </tr>
        ...
        <tfoot className="bg-gray-50 dark:bg-gray-750">
          <tr>
            ...
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce((total, branch) => total + calculateBranchAgeGroupTotal(branch, 'ages7to10'), 0)}
            </td>
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce((total, branch) => total + calculateBranchAgeGroupTotal(branch, 'ages11to14'), 0)}
            </td>
            <td className="px-1 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce((total, branch) => total + calculateBranchAgeGroupTotal(branch, 'adults'), 0)}
            </td>
          </tr>
        </tfoot>
      </table>
      ...
      <div className="mt-6">
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">First Communion Trends</h3>
        <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4 text-sm text-indigo-800 dark:text-indigo-300">
          <ul className="list-disc pl-5 space-y-1">
            <li>Most first communions occur in the 7-10 age group ({Math.round((branchesToDisplay.reduce((total, branch) => total + calculateBranchAgeGroupTotal(branch, 'ages7to10'), 0) / branchesToDisplay.reduce((total, branch) => total + calculateBranchTotal(branch), 0)) * 100)}%)</li>
            <li>March shows the highest number of first communions, likely due to seasonal preparation</li>
            <li>Adult first communions represent only {Math.round((branchesToDisplay.reduce((total, branch) => total + calculateBranchAgeGroupTotal(branch, 'adults'), 0) / branchesToDisplay.reduce((total, branch) => total + calculateBranchTotal(branch), 0)) * 100)}% of total first communions</li>
          </ul>
        </div>
      </div>
    </div>
