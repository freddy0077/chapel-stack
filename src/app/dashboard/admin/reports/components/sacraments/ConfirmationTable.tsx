...
export default function ConfirmationTable({
  ...
  const calculateMonthGroupTotal = (
    month: unknown,
    group: "youth" | "adults",
  ) => {
    return branchesToDisplay.reduce((total, branch) => {
  ...
  const calculateBranchGroupTotal = (
    branch: string,
    group: "youth" | "adults",
  ) => {
    return mockConfirmationData.reduce((total, month) => {
        ...
        <tfoot className="bg-gray-50 dark:bg-gray-750">
          <tr>
            ...
            <td className="px-2 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce(
                (total, branch) =>
                  total + calculateBranchGroupTotal(branch, "youth"),
                0,
              )}
            </td>
            <td className="px-2 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900 dark:text-white">
              {branchesToDisplay.reduce(
                (total, branch) =>
                  total + calculateBranchGroupTotal(branch, "adults"),
                0,
              )}
            </td>
          </tr>
        ...
        <div>
          ...
          <div className="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4 text-sm text-purple-800 dark:text-purple-300">
            <ul className="list-disc pl-5 space-y-1">
              <li>
                Youth confirmations account for{" "}
                {Math.round(
                  (branchesToDisplay.reduce(
                    (total, branch) =>
                      total + calculateBranchGroupTotal(branch, "youth"),
              ...
              <li>
                St. Mary\'s Cathedral has conducted the most confirmation
                ceremonies
              </li>
            </ul>
          </div>
