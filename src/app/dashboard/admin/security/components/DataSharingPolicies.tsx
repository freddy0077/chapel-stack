...
export default function DataSharingPolicies() {
  ...
  const [selectedPolicy, setSelectedPolicy] = useState<DataSharingPolicy | null>(null);
  ...
  const getEntityName = (entityType: 'branch' | 'ministry' | 'role', entityId: string): string => {
                ...
                <div className="flex justify-between">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-gray-500">
                      <span>
                        {getEntityName(policy.sourceType, policy.sourceId)} → {getEntityName(policy.targetType, policy.targetId)}
                      </span>
                    </div>
                  </div>
          ...
          {selectedPolicy ? (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-medium text-gray-900">{selectedPolicy.name}</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="bg-gray-50 p-4 rounded">
                  <h5 className="text-sm font-medium text-gray-700 mb-2">Source</h5>
                  {isEditing ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {selectedPolicy.sourceType === 'branch' && mockBranches.map(branch => (
                            <option key={branch.id} value={branch.id}>{branch.name}</option>
                          ))}
                          {selectedPolicy.sourceType === 'ministry' && mockMinistries.map(ministry => (
                            <option key={ministry.id} value={ministry.id}>{ministry.name}</option>
                          ))}
                          {selectedPolicy.sourceType === 'role' && mockRoles.map(role => (
                    ...
                    <div>
                      <p className="text-sm text-gray-900">
                        {selectedPolicy.sourceType.charAt(0).toUpperCase() + selectedPolicy.sourceType.slice(1)}:
                        {' '}{getEntityName(selectedPolicy.sourceType, selectedPolicy.sourceId)}
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        >
                          <option value="all">All</option>
                          {selectedPolicy.targetType === 'branch' && mockBranches.map(branch => (
                            <option key={branch.id} value={branch.id}>{branch.name}</option>
                          ))}
                          {selectedPolicy.targetType === 'ministry' && mockMinistries.map(ministry => (
                            <option key={ministry.id} value={ministry.id}>{ministry.name}</option>
                          ))}
                          {selectedPolicy.targetType === 'role' && mockRoles.map(role => (
                            <option key={role.id} value={role.id}>{role.name}</option>
                          ))}
                        </select>
                      </div>
                    ...
                    <div>
                      <p className="text-sm text-gray-900">
                        {selectedPolicy.targetType.charAt(0).toUpperCase() + selectedPolicy.targetType.slice(1)}:
                        {' '}{getEntityName(selectedPolicy.targetType, selectedPolicy.targetId)}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-gray-50 p-4 rounded mb-4">
                <h5 className="text-sm font-medium text-gray-700 mb-2">Shared Data & Permissions</h5>
                {isEditing ? (
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="mt-2 space-y-2">
                        <div className="flex items-center">
                          <input
                            ...
                            defaultChecked={selectedPolicy.permissions.includes('read')}
                          />
                        ...
                        <div className="flex items-center">
                          <input
                            ...
                            type="checkbox"
                            className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                            defaultChecked={selectedPolicy.permissions.includes('update')}
                          />
                          <label htmlFor="permission-update" className="ml-2 block text-sm text-gray-700">
                            Update
                          </label>
                        </div>
                        <div className="flex items-center">
                          <input
                            ...
                            type="checkbox"
                            className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                            defaultChecked={selectedPolicy.permissions.includes('delete')}
                          />
                  ...
                  <div>
                    <p className="text-sm text-gray-900">
                      ...
                      {selectedPolicy.resourceType === 'all' 
                        ? 'All Resources' 
                        : resourceTypes.find(r => r.id === selectedPolicy.resourceType)?.name || selectedPolicy.resourceType}
                    </p>
                    <div className="mt-2">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h4 className="text-lg font-medium text-gray-900">Create New Policy</h4>
                <button
                  type="button"
                  ...
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center">
                      <input
                        ...
                        defaultChecked={true}
                      />
                      <label htmlFor="new-permission-read" className="ml-2 block text-sm text-gray-700">
                        Read
                      </label>
                    </div>
                    <div className="flex items-center">
                      <input
                        ...
                        className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                      />
                      <label htmlFor="new-permission-update" className="ml-2 block text-sm text-gray-700">
                        Update
                      </label>
                    </div>
                    <div className="flex items-center">
                      <input
                        ...
                        className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                      />
                      <label htmlFor="new-permission-delete" className="ml-2 block text-sm text-gray-700">
                        Delete
                      </label>
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center h-full py-12">
              ...
              <p className="mt-2 text-sm text-gray-500">Select a policy to view details or create a new one</p>
            </div>
          )}
        </div>
      </div>
