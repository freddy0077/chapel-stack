...
function UserRoleEditor({
  ...
  useEffect(() => {
    const currentBranchIds = new Set(
      accessList.map((access) => access.branchId),
    );
    const remainingBranches = branches.filter(
      (branch) => !currentBranchIds.has(branch.id),
    );
  ...
  const addBranchAccess = () => {
    ...
    setAvailableBranches(
      availableBranches.filter((branch) => branch.id !== selectedBranchId),
    );

    // Reset form
    if (availableBranches.length > 1) {
      setSelectedBranchId(
        availableBranches.find((b) => b.id !== selectedBranchId)?.id || "",
      );
    } else {
      setSelectedBranchId("");
    }
  ...
  const removeBranchAccess = (branchId: string) => {
    const accessToRemove = accessList.find(
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <tr key={access.branchId} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button
                        ...
                        title={
                          accessList.length === 1
                            ? "Can't remove the last branch"
                            : "Remove branch access"
                        }
                      >
                        <TrashIcon
                          className={`h-5 w-5 ${accessList.length === 1 ? "opacity-50 cursor-not-allowed" : ""}`}
                        />
                      </button>
                    </td>
                  </tr>
      ...
      {assignError && (
        <div className="text-red-500 mt-2">Failed to assign role.</div>
      )}
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-3xl bg-white/90 p-8 text-left align-middle shadow-2xl ring-1 ring-indigo-100 transition-all">
                <Dialog.Title
                  as="h3"
                  className="text-2xl font-bold leading-7 text-indigo-900 mb-6 flex items-center gap-2"
                >
                  <PlusIcon className="h-7 w-7 text-indigo-500 bg-indigo-100 rounded-full p-1" />{" "}
                  Add New User
                </Dialog.Title>
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="mt-1 block w-full rounded-lg border border-indigo-200 bg-indigo-50/80 py-2 px-3 text-indigo-900 shadow-inner focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-indigo-700">
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    <button
                      type="button"
                      className="inline-flex justify-center rounded-lg border border-indigo-200 bg-white px-5 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                      onClick={onClose}
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      className="inline-flex justify-center rounded-lg border border-transparent bg-gradient-to-r from-indigo-500 to-blue-500 px-5 py-2 text-sm font-semibold text-white shadow hover:from-indigo-600 hover:to-blue-600 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                      disabled={loading}
                    >
                      Add User
                    </button>
...
export default function UserRoleAssignment() {
  ...
  const handleSaveUserRoles = (updatedAccessList: UserBranchAccess[]) => {
    ...
    const primaryBranchId =
      updatedAccessList.find((access) => access.isHomeBranch)?.branchId ||
      updatedAccessList[0]?.branchId;
    const roles = Array.from(
      new Set(updatedAccessList.map((access) => access.role)),
    ) as UserRole[];
  ...
  const getUserRoleLabel = (user: User): string => {
    ...
    const primaryBranchAccess = user.accessibleBranches?.find(
      (access) => access.isHomeBranch,
    );
    if (!primaryBranchAccess) return "Unknown";
    const roleLabel = availableRoles.find(
      (r) => r.value === primaryBranchAccess.role,
    )?.label;
