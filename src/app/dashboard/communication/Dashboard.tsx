...
const FilterIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
  </svg>
);
...
const MessageLineChart = ({ data }: { data: { date: string; count: number }[] }) => {
  if (!data || data.length === 0) {
    return <div className="h-32 w-full flex items-center justify-center text-gray-400">No message data available</div>;
  }
    ...
    <div className="relative h-32 w-full pt-2">
      ...
      <div className="flex items-end h-full justify-between gap-1 px-2">
        {data.slice(0, 7).map((item, index) => {
          ...
          return (
            <div key={index} className="flex flex-col items-center relative" style={{ height: '100%', flex: '1' }}>
              <div 
              ...
              <span className="text-[10px] text-gray-500 mt-1 truncate max-w-full">{format(date, 'MMM d')}</span>
            </div>
          );
        })}
      </div>
...
export default function CommunicationDashboard({ onCompose, onInbox }: { onCompose: () => void, onInbox: () => void }) {
  ...
  const toggleFilterItem = (item: string, selectedItems: string[], setSelectedItems: React.Dispatch<React.SetStateAction<string[]>>) => {
  ...
  const hasActiveFilters = searchQuery || selectedTypes.length > 0 || selectedStatuses.length > 0 || dateFilter !== 'last7days';
  ...
  const filteredMessages = React.useMemo(() => {
    ...
    return messages.filter(message => {
      ...
      if (searchQuery) {
        ...
        if (!subject.includes(query) && !body.includes(query) && !recipients.includes(query)) {
          return false;
        }
      }
  ...
  }, [messages, selectedTypes, selectedStatuses, searchQuery, startDate, endDate, messagesLoading]);
  ...
  const openModal = (msg: any) => {
    ...
    const messageWithType = {
      ...
      type: msg.type || (msg.__typename?.toLowerCase().includes('email') 
        ? 'email' 
        : msg.__typename?.toLowerCase().includes('sms') 
          ? 'sms' 
          : 'notification'),
      // Ensure body is properly set based on message type
      body: msg.body || msg.bodyText || (msg.bodyHtml ? stripHtmlTags(msg.bodyHtml) : ''),
      ...
      recipients: Array.isArray(msg.recipients) 
        ...
        : (typeof msg.recipients === 'string' 
          ? [msg.recipients] 
          : (msg.recipientCount && !msg.recipients ? Array(msg.recipientCount).fill('Recipient') : []))
    };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-800">Communication Dashboard</h2>
          <p className="text-gray-500 mt-1">Overview of your messaging activity and statistics</p>
        </div>
            ...
            <Card className="relative overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className="relative p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-medium text-gray-600">Email Messages</h3>
                    <p className="text-3xl font-bold">{stats?.totalEmailsSent?.toLocaleString() || 0}</p>
                  </div>
                </div>
                <div className="mt-4 border-t border-gray-100 pt-4 space-y-2">
                  <h4 className="text-sm font-semibold text-gray-700">Status Distribution</h4>
                  <StatusDistribution statusCounts={stats?.emailStatusCounts || []} type="email" />
                </div>
              </div>
            </Card>
            ...
            <Card className="relative overflow-hidden group hover:shadow-md transition-all">
              ...
              <div className="relative p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-medium text-gray-600">SMS Messages</h3>
                    <p className="text-3xl font-bold">{stats?.totalSmsSent?.toLocaleString() || 0}</p>
                  </div>
                </div>
                <div className="mt-4 border-t border-gray-100 pt-4 space-y-2">
                  <h4 className="text-sm font-semibold text-gray-700">Status Distribution</h4>
                  <StatusDistribution statusCounts={stats?.smsStatusCounts || []} type="sms" />
                </div>
              </div>
            </Card>
              ...
              <div className="relative p-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-3xl font-bold">{stats?.totalNotifications?.toLocaleString() || 0}</p>
                  </div>
                </div>
                <div className="mt-4 border-t border-gray-100 pt-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700">Active Templates</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 md:col-span-2 overflow-hidden">
          ...
          <p className="text-sm text-gray-500 mb-4">Distribution of messages sent over the past 7 days</p>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-lg font-bold text-gray-900">{stats?.deliveryRate || 0}%</span>
                </div>
                ...
                <p className="text-xs text-gray-500 mt-1">Percentage of messages successfully delivered</p>
              </div>
                  ...
                  <>
                    <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min(stats?.averageResponseTime / 60 * 100, 100)}%` }}
                      ></div>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">Average time for recipients to respond</p>
                  </>
                ) : (
                  <p className="text-xs text-gray-500">No response time data available</p>
                )}
              </div>
            </div>
          )}
          ...
          >
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              <div className="relative">
                <span className="absolute left-2 top-2.5 text-gray-400"><SearchIcon /></span>
                <input
                  type="text"
                  value={searchQuery}
                ...
                {['email', 'sms', 'notification'].map(type => (
                  <Button
                    ...
                    className={selectedTypes.includes(type)
                      ...
                      : 'border-gray-300'}
                    onClick={() => toggleFilterItem(type, selectedTypes, setSelectedTypes)}
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {['SENT', 'SCHEDULED', 'FAILED', 'DELIVERED'].map(status => (
                  <Button
                    ...
                    className={selectedStatuses.includes(status)
                      ...
                      : 'border-gray-300'}
                    onClick={() => toggleFilterItem(status, selectedStatuses, setSelectedStatuses)}
                  >
                    {status}
                  </Button>
                ))}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {selectedTypes.map(type => (
                <Badge 
                  ...
                  className={`bg-gradient-to-r ${
                    type === 'email' ? 'from-blue-500 to-violet-500' : 
                    type === 'sms' ? 'from-purple-500 to-pink-500' : 
              ...
              {selectedStatuses.map(status => (
                <Badge 
                  key={status}
                  className={`bg-gradient-to-r ${
                    status === 'SENT' || status === 'DELIVERED' ? 'from-green-500 to-emerald-600' : 
              ...
              <h3 className="font-medium text-gray-900">
                {!messagesLoading && filteredMessages.length > 0 && (
                  <span className="text-sm">
                    Showing <span className="font-semibold">{((currentPage - 1) * pageSize) + 1}-{Math.min(currentPage * pageSize, filteredMessages.length)}</span> of <span className="font-semibold">{filteredMessages.length}</span> messages
                  </span>
                )}
              </h3>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Subject/Content</th>
                ...
                {messagesLoading ? (
                  ...
                  paginatedMessages.map((message) => {
                    ...
                    if (message.type === 'email') {
                      ...
                      iconBgClass = 'bg-gradient-to-br from-blue-500 to-violet-600';
                    } else if (message.type === 'sms') {
                      icon = <SmsIcon className="h-4 w-4 text-white" />;
                      iconBgClass = 'bg-gradient-to-br from-purple-500 to-pink-600';
                    } else {
                      icon = <NotificationIcon className="h-4 w-4 text-white" />;
                                ...
                                {message.recipients ? (
                                  <span>
                                    To: {Array.isArray(message.recipients) 
                                      ...
                                      : (message.recipients ? 1 : 0)} recipient{Array.isArray(message.recipients) 
                                        ? (message.recipients.length !== 1 ? 's' : '') 
                                        : (message.recipients ? '' : 's')}
                                  </span>
                                ) : message.recipientCount ? (
                                  <span>To: {message.recipientCount} recipient{message.recipientCount !== 1 ? 's' : ''}</span>
                                ) : null}
                              </div>
                            </div>
                          </div>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-500">
                        ...
                        <p className="text-gray-500 text-sm mb-3">Try adjusting your filters or create a new message</p>
                        <Button 
                          onClick={onCompose} 
                          variant="default" 
          ...
          {filteredMessages.length > 0 && (
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end">
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, Math.ceil(filteredMessages.length / pageSize)) }, (_, i) => {
              ...
              <Button
                ...
                size="sm"
                className="border-gray-300 text-gray-700 hover:bg-gray-100"
                onClick={() => handlePageChange(Math.ceil(filteredMessages.length / pageSize))}
                disabled={currentPage >= Math.ceil(filteredMessages.length / pageSize)}
              >
                Last
              </Button>
          ...
          <Card className="overflow-hidden">
            ...
            <div className={`p-6 ${
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-sm font-medium text-white text-opacity-90 uppercase">{selectedMessage.type}</div>
                  <h3 className="text-xl font-bold">{selectedMessage.subject || 'No Subject'}</h3>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="flex flex-wrap gap-3 mb-6">
                ...
                {selectedMessage.createdAt && (
                  <Badge variant="outline" className="px-3 py-1 flex items-center gap-1 border-gray-300">
                ...
                {selectedMessage.scheduledAt && (
                  <Badge variant="outline" className="px-3 py-1 flex items-center gap-1 border-gray-300">
                    ...
                    Scheduled: {format(new Date(selectedMessage.scheduledAt), 'PPpp')}
                  </Badge>
                )}
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-md bg-gray-50 p-4 text-sm text-gray-800 whitespace-pre-line max-h-64 overflow-auto border border-gray-200">
                    {selectedMessage.body || <span className="italic text-gray-400">No content</span>}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-gray-50 rounded-md p-4 border border-gray-200">
                    {selectedMessage.recipients && Array.isArray(selectedMessage.recipients) && selectedMessage.recipients.length > 0 ? (
                      <>
                        <div className="flex flex-wrap gap-2">
                        ...
                        <div className="mt-2 text-xs text-gray-500">
                          Total: {selectedMessage.recipients.length} recipient(s)
                        </div>
                      </>
                    ) : (
                      <div className="text-sm text-gray-500">No recipients found</div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-gray-50 rounded-md p-4 grid grid-cols-1 md:grid-cols-2 gap-4 border border-gray-200">
                    <div>
                      ...
                      <div className="text-sm font-medium text-gray-700">{selectedMessage.id || 'N/A'}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">Created By</div>
                      <div className="text-sm font-medium text-gray-700">{selectedMessage.createdBy || 'System'}</div>
                    </div>
                    {selectedMessage.templateId && (
                      <div>
                        <div className="text-xs text-gray-500">Template</div>
                        <div className="text-sm font-medium text-gray-700">{selectedMessage.templateId}</div>
                      </div>
                    )}
                    ...
                    {selectedMessage.openRate !== undefined && (
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-gray-700">{selectedMessage.openRate}%</span>
                        </div>
                      </div>
                    )}
                    {selectedMessage.clickRate !== undefined && (
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-gray-700">{selectedMessage.clickRate}%</span>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {process.env.NODE_ENV !== 'production' && (
                  <div className="mt-6 p-4 border border-amber-300 bg-amber-50 rounded-md">
                    <div className="text-sm font-medium text-amber-800 mb-2">Debug Information</div>
                    <div className="text-xs text-amber-700 overflow-auto max-h-40">
                      <div><strong>Message Type:</strong> {selectedMessage.type}</div>
                      <div><strong>Has Recipients Array:</strong> {Array.isArray(selectedMessage.recipients) ? 'Yes' : 'No'}</div>
                      <div><strong>Recipients Length:</strong> {Array.isArray(selectedMessage.recipients) ? selectedMessage.recipients.length : 'N/A'}</div>
                      <div><strong>Recipients Data:</strong> {JSON.stringify(selectedMessage.recipients)}</div>
                      <div className="mt-2"><strong>Full Message Data:</strong></div>
                      <pre className="whitespace-pre-wrap">{JSON.stringify(selectedMessage, null, 2)}</pre>
                    </div>
                  </div>
                )}
              </div>
