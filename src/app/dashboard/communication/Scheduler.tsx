    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage your scheduled and automated messages.</p>
        </div>
        <Button className="bg-violet-600 text-white hover:bg-violet-700">Schedule a New Message</Button>
      </div>

      {/* Scheduled Messages List */}
      <Card className="shadow-sm">
        <div className="p-6">
          <h3 className="text-xl font-bold text-gray-800">Upcoming & Recent Messages</h3>
        </div>
              ...
              {SCHEDULED_MESSAGES.map((msg) => (
                <tr key={msg.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <Badge className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[msg.status]}`}>{msg.status}</Badge>
                  </td>
