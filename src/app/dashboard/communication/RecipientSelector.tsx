...
const BirthdayIcon = () => (
  <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
    <path strokeLinecap="round" strokeLinejoin="round" d="M21 15.546c-.523 0-1.046.151-1.5.454a2.704 2.704 0 01-3 0 2.704 2.704 0 00-3 0 2.704 2.704 0 01-3 0 2.701 2.701 0 00-1.5-.454M9 6v2m3-2v2m3-2v2M9 3h.01M12 3h.01M15 3h.01M21 21v-7a2 2 0 00-2-2H5a2 2 0 00-2 2v7h18zm-3-9v-2a2 2 0 00-2-2H8a2 2 0 00-2 2v2h12z" />
  </svg>
);

export default function RecipientSelector({ recipients, setRecipients, birthdayRange, setBirthdayRange }) {
  const [mode, setMode] = useState('groups');
  const [searchTerm, setSearchTerm] = useState('');
  const [showFilters, setShowFilters] = useState(false);
  const { groups, loading: groupsLoading, error: groupsError } = useRecipientGroups();
  const { members: searchMembers, searchMembers: triggerSearch, loading: searchLoading } = useMemberSearch();
  ...
  const [autoSelectBirthdaysTrigger, setAutoSelectBirthdaysTrigger] = useState('');
  useEffect(() => {
    if (mode === 'birthdays' && birthdayMembers.length > 0) {
      // Add all birthdayMembers not already in recipients
      setRecipients(prev => {
        const existingIds = new Set(prev.map(r => r.id));
        const newBirthday = birthdayMembers.filter(m => !existingIds.has(m.id));
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Select who should receive this message</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="bg-gray-100 rounded-full px-4 py-1 text-sm font-medium">
            {recipientCounts.groups + recipientCounts.individuals + recipientCounts.filters} selected
          </div>
        </div>
      </div>
      
      {/* Selection area */}
      <Tabs defaultValue="groups" className="w-full" onValueChange={setMode} value={mode}>
        <TabsList className="grid grid-cols-4 mb-6">

          <TabsTrigger value="filter" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-500 data-[state=active]:to-fuchsia-500 data-[state=active]:text-white">
            <FilterIcon />
            <span className="ml-2">Filter</span>
          </TabsTrigger>
          <TabsTrigger value="groups" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-500 data-[state=active]:to-fuchsia-500 data-[state=active]:text-white rounded-l-lg">
            <GroupsIcon />
            <span className="ml-2">Groups</span>
          </TabsTrigger>
          <TabsTrigger value="search" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-500 data-[state=active]:to-fuchsia-500 data-[state=active]:text-white">
            <SearchIcon />
            <span className="ml-2">Search</span>
          </TabsTrigger>

          <TabsTrigger value="birthdays" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-500 data-[state=active]:to-fuchsia-500 data-[state=active]:text-white rounded-r-lg">
            <BirthdayIcon />
            ...
            <div className="transition-opacity duration-200">
              {groupsLoading ? (
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-600">Select groups to message all their members:</p>
                    ...
                    {groups.map(group => (
                      ...
                      >
                        <div 
                          ...
                          `}
                          onClick={() => handleAddRecipient({ ...group, type: 'Group' })}
                        >
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={e => {
                    ...
                    if (e.target.value.length > 1) triggerSearch({ variables: { query: e.target.value } });
                  }}
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                {searchLoading ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    {searchTerm.length > 1 ? "No members found" : "Type to search members"}
                  </div>
                ) : (
                  <div className="divide-y divide-gray-100 max-h-64 overflow-y-auto">
                    {searchMembers.map(member => (
                      <div
                        ...
                        `}
                        onClick={() => handleAddRecipient({ ...member, type: 'Member' })}
                      >
            ...
            <div className="transition-opacity duration-200 space-y-5">
              <div className="flex flex-wrap gap-2">
                <Button 
                  ...
                  className={birthdayRange === 'TODAY' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-500' : ''}
                >
                  Today
                </Button>
                <Button 
                  size="sm" 
                  variant={birthdayRange === 'THIS_WEEK' ? "default" : "outline"}
                  onClick={() => handleBirthdayRange('THIS_WEEK')}
                  className={birthdayRange === 'THIS_WEEK' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-500' : ''}
                >
                  This Week
                </Button>
                <Button 
                  size="sm" 
                  variant={birthdayRange === 'THIS_MONTH' ? "default" : "outline"}
                  onClick={() => handleBirthdayRange('THIS_MONTH')}
                  className={birthdayRange === 'THIS_MONTH' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-500' : ''}
                >
                  This Month
                </Button>
              </div>
                    ...
                    <div className="bg-violet-50 border border-violet-200 rounded-lg p-3 text-sm">
                      <div className="flex items-center">
                        ...
                        <span className="font-medium text-violet-700">
                          {birthdayMembers.length} {birthdayMembers.length === 1 ? 'person' : 'people'} have birthdays {birthdayRange === 'TODAY' ? 'today' : birthdayRange === 'THIS_WEEK' ? 'this week' : 'this month'}
                        </span>
                      </div>
                      <div className="text-gray-600 mt-1 text-sm">
                        All birthday recipients have been automatically added to your selection
                      </div>
                    </div>
                  )}
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                      <div className="divide-y divide-gray-100 max-h-48 overflow-y-auto">
                        {birthdayMembers.map(member => (
                          <div key={member.id} className="flex items-center justify-between px-4 py-2 hover:bg-gray-50">
                            <div className="flex items-center space-x-3">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'parents');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'parents')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-parents', type: 'Filter', key: 'parents', label: 'Parents & Families' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    <div className="h-8 w-8 rounded-md bg-gradient-to-br from-violet-500 to-fuchsia-500 text-white flex items-center justify-center mr-3">
                      <span role="img" aria-label="Parents">👨‍👩‍👧‍👦</span>
                    </div>
                    <div className="flex-1 truncate">
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'newsletter');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'newsletter')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-newsletter', type: 'Filter', key: 'newsletter', label: 'Newsletter Subscribers' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Newsletter Subscribers</div>
                      <div className="text-xs text-gray-500 truncate">Opted-in for newsletters</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'newsletter') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'anniversary');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'anniversary')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-anniversary', type: 'Filter', key: 'anniversary', label: 'Anniversary Celebrants' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    <div className="h-8 w-8 rounded-md bg-gradient-to-br from-violet-500 to-fuchsia-500 text-white flex items-center justify-center mr-3">
                      <span role="img" aria-label="Anniversary">🎉</span>
                    </div>
                    <div className="flex-1 truncate">
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'baptized');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'baptized')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-baptized', type: 'Filter', key: 'baptized', label: 'Recently Baptized' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    <div className="h-8 w-8 rounded-md bg-gradient-to-br from-violet-500 to-fuchsia-500 text-white flex items-center justify-center mr-3">
                      <span role="img" aria-label="Baptized">💧</span>
                    </div>
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Recently Baptized</div>
                      <div className="text-xs text-gray-500 truncate">Members baptized recently</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'baptized') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'milestone');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'milestone')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-milestone', type: 'Filter', key: 'milestone', label: 'Milestone Achievers' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Milestone Achievers</div>
                      <div className="text-xs text-gray-500 truncate">Special recognitions</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'milestone') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Custom List (CSV/Manual)</div>
                      <div className="text-xs text-gray-500 truncate">Upload or paste emails</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'custom') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">By Demographics</div>
                      <div className="text-xs text-gray-500 truncate">Filter by age, gender, location</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'demographics') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Inactive Members</div>
                      <div className="text-xs text-gray-500 truncate">Not active recently</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'inactive') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'prayer');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'prayer')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-prayer', type: 'Filter', key: 'prayer', label: 'Prayer Request Submitters' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Prayer Request Submitters</div>
                      <div className="text-xs text-gray-500 truncate">People who submitted prayers</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'prayer') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'guests');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'guests')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-guests', type: 'Filter', key: 'guests', label: 'Guests & Visitors' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    <div className="h-8 w-8 rounded-md bg-gradient-to-br from-violet-500 to-fuchsia-500 text-white flex items-center justify-center mr-3">
                      <span role="img" aria-label="Guests">🎟️</span>
                    </div>
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Guests & Visitors</div>
                      <div className="text-xs text-gray-500 truncate">Recent church visitors</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'guests') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'donors');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'donors')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-donors', type: 'Filter', key: 'donors', label: 'Donors' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Donors</div>
                      <div className="text-xs text-gray-500 truncate">Financial supporters</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'donors') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                <div
                  ...
                  onClick={() => {
                    const exists = recipients.find(r => r.type === 'Filter' && r.key === 'staff');
                    if (exists) {
                      setRecipients(recipients.filter(r => !(r.type === 'Filter' && r.key === 'staff')));
                    } else {
                      setRecipients([...recipients, { id: 'filter-staff', type: 'Filter', key: 'staff', label: 'Staff' }]);
                    }
                  }}
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Staff</div>
                      <div className="text-xs text-gray-500 truncate">Church staff and leaders</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'staff') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">Volunteers</div>
                      <div className="text-xs text-gray-500 truncate">Active volunteers</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'volunteers') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">New Members</div>
                      <div className="text-xs text-gray-500 truncate">Recently joined</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'new-members') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 truncate">
                      <div className="font-medium text-gray-900 truncate">All Members</div>
                      <div className="text-xs text-gray-500 truncate">Everyone in the system</div>
                    </div>
                    {recipients.find(r => r.type === 'Filter' && r.key === 'all-members') && (
                      <div className="ml-2 text-violet-600">
                        <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              ...
              <p className="text-xs text-gray-500 mt-2">These filters let you target special groups not covered by Groups, Search, or Birthdays.</p>
            </div>
          </TabsContent>
        </div>
      </Tabs>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-sm text-gray-500">
              {recipientCounts.groups} groups, {recipientCounts.individuals} individuals, {recipientCounts.filters} filters
            </div>
          </div>
            ...
            {recipients.map(recipient => (
              ...
              >
                ...
                >
                  ...
                  {recipient.type === 'Group' ? recipient.name : recipient.type === 'Member' ? `${recipient.firstName} ${recipient.lastName}` : recipient.label}
                  <button
      ...
      {/* Modals for additional info */}
      <DemographicsModal open={demographicsOpen} onClose={() => setDemographicsOpen(false)} onSave={data => {
        setRecipients([...recipients.filter(r => !(r.type === 'Filter' && r.key === 'demographics')), { id: 'filter-demographics', type: 'Filter', key: 'demographics', label: `By Demographics: ${data.summary}` }]);
        setDemographicsOpen(false);
      }} />
      <EventAttendeesModal open={eventAttendeesOpen} onClose={() => setEventAttendeesOpen(false)} onSave={data => {
        setRecipients([...recipients.filter(r => !(r.type === 'Filter' && r.key === 'event-attendees')), { id: 'filter-event-attendees', type: 'Filter', key: 'event-attendees', label: `Event: ${data.eventName}` }]);
        setEventAttendeesOpen(false);
      }} />
      <CustomListModal open={customListOpen} onClose={() => setCustomListOpen(false)} onSave={data => {
        setRecipients([...recipients.filter(r => !(r.type === 'Filter' && r.key === 'custom')), { id: 'filter-custom', type: 'Filter', key: 'custom', label: `Custom List (${data.count} emails)` }]);
