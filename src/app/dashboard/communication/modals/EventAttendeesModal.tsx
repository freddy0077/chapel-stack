...
const EventAttendeesModal: React.FC<EventAttendeesModalProps> = ({ open, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded shadow-lg w-full max-w-md p-6 animate-fade-in">
        ...
        <div className="mb-2 text-sm text-gray-700">Enter or select the event name:</div>
        <input
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button className="btn btn-primary" onClick={handleSave} disabled={!eventName}>Save</button>
        </div>
      </div>
    </div>
  );
