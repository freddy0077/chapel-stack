...
export default function AttendanceReportsDashboard() {
  const [activeTab, setActiveTab] = useState<
    "consolidated" | "branch-comparison" | "visitor-flow"
  >("consolidated");
  ...
  const filteredAttendance = mockAttendanceRecords.filter((record) => {
    ...
    const matchesBranch =
      selectedBranch === "" || record.branchId === selectedBranch;
  ...
  const totalAttendees = filteredAttendance.reduce(
    (sum, record) => sum + record.attendeeCount,
    0,
  );
  const totalVisitors = filteredAttendance.reduce(
    (sum, record) => sum + record.visitorCount,
    0,
  );
  const totalServices = filteredAttendance.length;

  const averageAttendance =
    totalServices > 0 ? Math.round(totalAttendees / totalServices) : 0;
  const averageVisitors =
    totalServices > 0 ? Math.round(totalVisitors / totalServices) : 0;

  // Calculate attendance by branch
  const attendanceByBranch = mockBranches
    .map((branch) => {
      const branchRecords = filteredAttendance.filter(
        (record) => record.branchId === branch.id,
      );
      const totalAttendees = branchRecords.reduce(
        (sum, record) => sum + record.attendeeCount,
        0,
      );
                      ...
                      <tr key={record.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm text-gray-500">
                            {record.eventType === "SundayService"
                              ? "Sunday Service"
                              : record.eventType}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-500">
                            {record.adultsCount} adults, {record.youthCount}{" "}
                            youth, {record.childrenCount} children
                          </div>
                        </td>
                      </tr>
                    ))}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700">
                      {branch.totalServices} service
                      {branch.totalServices !== 1 ? "s" : ""}
                    </span>
                  </div>
                    ...
                    <div className="mt-4">
                      <div className="relative pt-1">
                        <div className="flex mb-2 items-center justify-between">
                          <div>
                            <span className="text-xs font-semibold inline-block text-blue-600">
                              {Math.round(
                                (branch.totalAttendees / totalAttendees) * 100,
                              )}
                              % of total
                            </span>
                          </div>
                        ...
                        <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-blue-100">
                          <div
                            style={{
                              width: `${(branch.totalAttendees / totalAttendees) * 100}%`,
                            }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${
                        flow.status === "First Visit"
                          ? "bg-green-50 text-green-700"
                          : flow.status === "Return Visit"
