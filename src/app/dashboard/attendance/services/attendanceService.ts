...
export const recordAttendance = async (
  ...
  const existingRecord = mockAttendanceRecords.find(
    record => record.memberId === memberId && record.eventId === eventId && 
...
export const checkOutAttendee = async (
  ...
  if (!record) {
    throw new Error(`Attendance record with ID ${attendanceRecordId} not found`);
  }
...
export const getEventAttendanceStats = async (
  ...
  const manualEntryCount = records.filter(r => r.method === 'manual_entry').length;
  const mobileAppCount = records.filter(r => r.method === 'mobile_app').length;
...
export const getAttendanceTrends = async (
  ...
  let filteredEvents = mockEvents.filter(e => 
  ...
  const overallGrowth = (lastMonthAttendance - firstMonthAttendance) / firstMonthAttendance * 100;
...
export const getAbsenceAlerts = async (
  ...
  const absentMembers = mockMembers.filter(member => 
  ...
  const alerts: AbsenceAlert[] = absentMembers.map(member => {
    ...
    const daysSinceLastAttendance = Math.floor((today.getTime() - lastDate.getTime()) / (1000 * 60 * 60 * 24));
...
export const findAvailableDevicesForEvent = async (
  ...
  const overlappingEvents = mockEvents.filter(event => 
    (event.startTime <= endTime && event.endTime >= startTime) &&
    event.status !== 'cancelled'
  );
  
  const assignedDeviceIds = overlappingEvents.flatMap(e => e.assignedDevices || []);
...
export const simulateCardScan = async (
  ...
  // Find the card
  const card = mockCards.find(c => c.cardNumber === cardNumber && c.status === 'active');
  if (!card) {
