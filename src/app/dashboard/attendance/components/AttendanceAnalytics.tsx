...
export default function AttendanceAnalytics() {
  ...
  const { analytics, loading, error } = useAttendanceAnalytics(
    analyticsInput,
  ...
  const totalAttendees =
    analytics?.TOTAL_ATTENDANCE?.reduce(
      (sum: number, d: { total?: number }) => sum + (d.total || 0),
      0,
    ) || 0;
  const uniqueMembers =
    analytics?.UNIQUE_MEMBERS?.reduce(
      (sum: number, d: { unique_members?: number }) =>
        sum + (d.unique_members || 0),
      0,
    ) || 0;
  const visitors =
    analytics?.VISITORS?.reduce(
      (sum: number, d: { visitors?: number }) => sum + (d.visitors || 0),
      0,
    ) || 0;
  const firstTimeVisitors =
    analytics?.FIRST_TIME_VISITORS?.reduce(
      (sum: number, d: { first_time_visitors?: number }) =>
        sum + (d.first_time_visitors || 0),
      0,
    ) || 0;
  ...
  const attendanceTrendData = useMemo(() => {
    if (
      ...
      Array.isArray(analytics.TOTAL_ATTENDANCE) &&
      analytics.TOTAL_ATTENDANCE.length > 0
    ) {
      return {
        ...
        datasets: [
          {
            label: "Attendance",
            data: analytics.TOTAL_ATTENDANCE.map((d: any) =>
                ...
                <div className="flex-shrink-0 bg-green-100 rounded-md p-3">
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M12 8c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm0 2c-2.21 0-4 1.79-4 4v4h8v-4c0-2.21-1.79-4-4-4z"
                    />
                  </svg>
                </div>
                <div className="ml-5 w-0 flex-1">
                ...
                <div className="flex-shrink-0 bg-blue-100 rounded-md p-3">
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M8 7a4 4 0 018 0v4a4 4 0 01-8 0V7z"
                    />
                  </svg>
                </div>
                <div className="ml-5 w-0 flex-1">
