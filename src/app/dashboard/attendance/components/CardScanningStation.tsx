...
export default function CardScanningStation({
  deviceId,
  onAttendanceRecorded,
}: CardScanningStationProps) {
  ...
  const handleScanCard = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        setMessage(
          response.errors[0].message || "Scan failed. Please try again.",
        );
        setScanStatus("error");
      } else {
