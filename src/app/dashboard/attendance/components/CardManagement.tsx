...
export default function CardManagement() {
  ...
  const { assignRfidCardToMember, loading: issuing, error: issueError, data: issueData } = useAssignRfidCardToMember();
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Member Card Management</h2>
        <button
  ...
  cards
    .filter(card => {
      ...
      const memberName = `${card.firstName} ${card.lastName}`.toLowerCase();
      return (
        (card.rfidCardId || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <React.Fragment key={card.id}>
      <tr
        ...
        onClick={() => setExpandedRowId(expandedRowId === card.id ? null : card.id)}
      >
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <div className="text-sm text-gray-900">{card.cardNumber}</div>
          <div className="text-sm text-gray-500 capitalize">{card.cardType} Card</div>
        </td>
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeColor(card.status)}`}>
        ...
        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
          <button
            ...
            onClick={e => { e.stopPropagation(); setExpandedRowId(expandedRowId === card.id ? null : card.id); }}
            aria-label={expandedRowId === card.id ? 'Collapse details' : 'Expand details'}
          >
            {expandedRowId === card.id ? <ArrowPathIcon className="w-5 h-5 inline" /> : <PlusCircleIcon className="w-5 h-5 inline" />}
          </button>
        </td>
      </tr>
      ...
      {expandedRowId === card.id && (
        <tr>
          <td colSpan={6} className="bg-gray-50 px-6 py-6 border-b">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <div className="font-semibold text-gray-700 flex items-center mb-2"><CheckCircleIcon className="w-4 h-4 mr-1 text-green-500" /> Card & Member Info</div>
                <ul className="text-sm text-gray-600 space-y-1">
                  <li><span className="font-medium">Full Name:</span> {card.firstName + ' ' + card.lastName}</li>
                  <li><span className="font-medium">Email:</span> {card.email || '—'}</li>
                  <li><span className="font-medium">Phone:</span> {card.phoneNumber || '—'}</li>
              ...
              <div>
                <div className="font-semibold text-gray-700 flex items-center mb-2"><ArrowPathIcon className="w-4 h-4 mr-1 text-blue-500" /> Relationships & Branch</div>
                <ul className="text-sm text-gray-600 space-y-1">
                  <li><span className="font-medium">Branch:</span> {card.branch?.name || '—'}</li>
                  <li><span className="font-medium">Spouse:</span> {card.spouse ? `${card.spouse.firstName} ${card.spouse.lastName}` : '—'}</li>
                  <li><span className="font-medium">Children:</span> {card.children && card.children.length > 0 ? card.children.map(child => `${child.firstName} ${child.lastName}`).join(', ') : '—'}</li>
                  <li><span className="font-medium">Parent:</span> {card.parent ? `${card.parent.firstName} ${card.parent.lastName}` : '—'}</li>
                  <li><span className="font-medium">Address:</span> {card.address || '—'}, {card.city || ''} {card.state || ''} {card.country || ''}</li>
                </ul>
              </div>
            </div>
          </td>
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Inactive Cards</dt>
                  <dd className="flex items-baseline">
                    <div className="text-2xl font-semibold text-gray-900">
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Lost Cards</dt>
                  <dd className="flex items-baseline">
                    <div className="text-2xl font-semibold text-gray-900">
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Members Without Cards</dt>
                  <dd className="flex items-baseline">
                    <div className="text-2xl font-semibold text-gray-900">
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            ...
            <div className="mb-6">
              ...
              <p className="mt-1 text-xs text-gray-500">
                For RFID/NFC cards, enter the physical card number. For QR codes, provide a unique identifier.
              </p>
            </div>
