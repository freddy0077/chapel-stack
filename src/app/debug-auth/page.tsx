    ...
    <div className="p-8">
      ...
      <div className="space-y-4">
        <div className="bg-gray-100 p-4 rounded">
          ...
          <p><strong>Is Authenticated:</strong> {state.isAuthenticated ? 'Yes' : 'No'}</p>
          <p><strong>Is Loading:</strong> {state.isLoading ? 'Yes' : 'No'}</p>
          <p><strong>Is Hydrated:</strong> {state.isHydrated ? 'Yes' : 'No'}</p>
        ...
        {user && (
          <div className="bg-blue-100 p-4 rounded">
            ...
            <p><strong>Organisation ID:</strong> {user.organisationId || 'MISSING/NULL'}</p>
            <p><strong>Primary Role:</strong> {user.primaryRole}</p>
            <p><strong>Roles:</strong> {user.roles?.join(', ')}</p>
            <p><strong>User Branches:</strong> {user.userBranches?.length || 0}</p>
          </div>
        )}

        <div className="bg-yellow-100 p-4 rounded">
          <h2 className="font-semibold">Raw Data (Check Console)</h2>
          <p>Check the browser console for detailed logging of the auth state and localStorage data.</p>
        </div>
      </div>
    </div>
  );
